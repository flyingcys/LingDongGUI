...
void *picoui_backend_create_list(void *parent, const char *id)
{
    ...
    if (picoui_backend_widget_init_child(widget,
                                         ...
                                         id,
                                         ((struct picoui_backend_widget *)parent)->theme) != 0) {
...
int picoui_backend_list_set_item_widget(void *backend_widget, int index, void *item_widget_backend)
{
    ...
    if (item_widget->parent != NULL && item_widget->parent != list_widget) {
        if (picoui_widget_backend_detach(item_widget) != 0) {
            return -1;
        }
    ...
    if (picoui_backend_widget_attach_child(list_widget, item_widget) != 0) {
        return -1;
    }

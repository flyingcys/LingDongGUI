...
TINYUI 是构建在 LingDongGUI 之上的应用层抽象，提供统一的 Linux 风格 `tinyui_*` API。

- 用户不需要直接使用 `ld*`
- 用户不需要直接使用 `ARM-2D`
- 推荐从 `tinyui/demo/hello_world` 开始
- 进一步可查看 `tinyui/docs/quick_start.md` 与 `tinyui/docs/api_overview.md`
...
| `image` 对象映射与 source 绑定 | 已完成 | `tinyui_image_source` 已能绑定到底层 `ldImage` |
...
- `tests/tinyui/runtime/check_tinyui_runtime.py` 会使用独立的 `build/tinyui-runtime` 目录做 TINYUI demo runtime 检查
- `examples/sdl` 子目录 configure 仍可用于 SDL demo 定向调试，但不再是主推荐入口

### 当前口径提醒

- `TINYUI` 的主线目标已经收口到“真实 `TINYUI -> LingDongGUI` backend 映射”，不是把 SDL host 做成第二套 GUI 渲染器
- `tests/tinyui/runtime/check_tinyui_runtime.py` 与 capture 结果仍只属于 smoke / 回归证据

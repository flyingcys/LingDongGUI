...
static int test_qrcode_finish_detach_after_backend_failure(struct picoui_backend_widget *backend)
{
...
static void test_qrcode_fill_snapshot(struct picoui_backend_widget *backend,
                                      int detach_result,
                                      int unbind_result)
{
    ldBase_t *ld_base = backend != 0 ? (ldBase_t *)backend->ld_widget : 0;

    g_qrcode_snapshot.kind = backend != 0 ? backend->kind : -1;
    ...
    g_qrcode_snapshot.detached = (detach_result == 0 && backend != 0 && backend->parent == 0);
    g_qrcode_snapshot.owner_cleared = (backend != 0 && backend->owner == 0);
    g_qrcode_snapshot.root_cleared = (backend != 0 && backend->root == 0);
    g_qrcode_snapshot.parent_cleared = (backend != 0 && backend->parent == 0);
    g_qrcode_snapshot.next_sibling_cleared = (backend != 0 && backend->next_sibling == 0);
    g_qrcode_snapshot.host_cleared = (backend != 0 && backend->host_widget == 0);
    g_qrcode_snapshot.event_bridge_cleared = (backend != 0
    ...
    const struct picoui_qrcode_props *props)
{
    ...
    if (backend->parent != 0) {
        ...
        if (detach_result != 0) {
            detach_result = test_qrcode_finish_detach_after_backend_failure(backend);
        }
    }
    unbind_result = picoui_backend_widget_unbind_host(backend);
    test_qrcode_fill_snapshot(backend, detach_result, unbind_result);
    picoui_widget_destroy(&qrcode->widget);
    return 0;
}
...
static void test_qrcode_create_and_props(struct picoui_window *win)
{
    ...
    assert(picoui_widget_has_ld_binding(&qrcode->widget) == 1);
...
static void test_qrcode_create_with_props_failure_rolls_back_attached_child(struct picoui_window *win)
{
    ...
    assert(picoui_backend_qrcode_test_create_with_props_fail_before_text(
               (struct picoui_widget *)win,
               &(struct picoui_qrcode_props){
                   .id = "qr_fail_text",
                   .style_class = "qr-fail",
           ...
           == 0);
    assert(picoui_backend_qrcode_test_take_last_dispose_snapshot(&snapshot) == 0);
    ...
    assert(picoui_backend_qrcode_test_take_last_dispose_snapshot(&snapshot) == -1);
    if (tail != 0) {

...
static void on_keyboard_event(struct picoui_keyboard *keyboard,
...
static void on_keyboard_draw(struct picoui_keyboard *keyboard,
...
static void test_keyboard_event_capture(struct picoui_keyboard *keyboard,
                                        unsigned int key_code,
                                        unsigned int signal,
                                        void *user_data)
{
...
static void test_keyboard_draw_callback_round_trip(void)
{
    ...
    struct picoui_window *win;
    struct picoui_keyboard *keyboard;
    static const struct picoui_keyboard_button custom_buttons[] = {
    ...
    win = test_window_create(&app);
    keyboard = picoui_keyboard_create(win, "keyboard_draw_callback");

    assert(keyboard != 0);
    assert(picoui_keyboard_set_layout(keyboard, custom_buttons, 2) == 0);
    assert(picoui_keyboard_set_draw_callback(0, on_keyboard_draw, &keyboard_draw_cookie) == -1);
    assert(picoui_keyboard_set_draw_callback(keyboard, on_keyboard_draw, &keyboard_draw_cookie) == 0);
    assert(picoui_keyboard_update(keyboard) == 0);
    assert(keyboard_draw_count == 2);
    assert(keyboard_draw_last_key_code == 'b');
    assert(picoui_keyboard_set_draw_callback(keyboard, 0, 0) == 0);
    picoui_app_destroy(app);
}
...
static void test_keyboard_set_layout_replaces_native_button_list(void)
{
    ...
    struct picoui_backend_widget *backend;
    ldKeyboard_t *ld_keyboard;
    static const struct picoui_keyboard_button buttons[] = {
    ...
    win = test_window_create(&app);
    keyboard = picoui_keyboard_create(win, "keyboard_custom_layout");
    line_edit = picoui_line_edit_create(win, "line_edit_custom_layout");
    ...
    assert(picoui_keyboard_set_buttons(keyboard, buttons, 2) == 0);
    assert(picoui_widget_claim_focus(&line_edit->widget) == 0);
    assert(picoui_keyboard_update(keyboard) == 0);
    assert(ld_keyboard->pBtnList != 0);
    ...
    assert(picoui_keyboard_set_buttons(keyboard, 0, 0) == 0);
    assert(picoui_keyboard_set_buttons(keyboard, buttons, 0) == 0);
    picoui_app_destroy(app);
}
...
static void test_keyboard_callback_observes_button_update_and_click(void)
{
    ...
    win = test_window_create(&app);
    keyboard = picoui_keyboard_create(win, "keyboard_callback");
    line_edit = picoui_line_edit_create(win, "line_edit_callback");
    ...
    assert(picoui_widget_claim_focus(&line_edit->widget) == 0);
    assert(picoui_widget_claim_focus(&keyboard->widget) == 0);
    assert(picoui_keyboard_button_update(keyboard, '8') == 0);
    assert(capture[0] == '8');
    assert(capture[1] == PICOUI_NATIVE_SIGNAL_VALUE_CHANGED);
    assert(capture[2] == 1U);
    assert(picoui_keyboard_click(keyboard) == 0);
    assert(capture[0] == '8');
    assert(capture[1] == PICOUI_NATIVE_SIGNAL_PRESS);
    assert(capture[2] == 2U);
    assert(picoui_keyboard_set_on_key_event(keyboard, 0, 0) == 0);
    assert(picoui_keyboard_button_update(keyboard, '9') == 0);
    assert(capture[2] == 2U);
    picoui_app_destroy(app);
}
...
static void test_keyboard_custom_layout_round_trips_into_native_button_table(void)
{
    ...
    const struct picoui_keyboard_button *round_trip = 0;
    int button_count = -1;
    ...
    assert(picoui_keyboard_set_buttons(keyboard, custom_buttons, 2) == 0);
    assert(picoui_keyboard_get_buttons(keyboard, &round_trip, &button_count) == 0);
...
static void test_keyboard_native_press_and_release_emit_picoui_callback(void)
{
    struct picoui_app *app;
    struct picoui_window *win;
    struct picoui_keyboard *keyboard;
    ...
    win = test_window_create(&app);
    keyboard = picoui_keyboard_create(win, "keyboard_event_bridge");
    assert(keyboard != 0);
    assert(picoui_keyboard_set_on_key_event(0, on_keyboard_event, &keyboard_event_cookie) == -1);
    assert(picoui_keyboard_set_on_key_event(keyboard, on_keyboard_event, &keyboard_event_cookie) == 0);
...
int main(void)
{
    test_self_binary_path = "tests/picoui/test_tinyui_keyboard";

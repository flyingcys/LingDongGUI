...
| 6 | `ldKeyboardGetTargetBtnList` | `getter` | `widget` | `direct_covered` |  | `true` | `covered` | `shared_api_equivalence` | `picoui_keyboard_set_layout` | `picoui_backend_keyboard_prepare_custom_layout` | test_keyboard_custom_layout_button_table_round_trip, ctest:test_picoui_keyboard | a-0.13 item 1 已通过 portable keyboard layout API 覆盖自定义按钮表/布局用户意图，不直接暴露 native `kbBtnInfo_t`。 | `extern const kbBtnInfo_t *ldKeyboardGetTargetBtnList(ldKeyboard_t *ptWidget);` |

# PicoUI Text Font 合同线设计

## 目标

- 为 `text | picoui_text_set_font()` 定义可实现、可验证、且不扭曲现有 public API 的真实合同。
- 明确 `struct picoui_font { family, size }` 在 PicoUI 中的语义边界，避免继续把 wrapper-only 指针存储误写成完整 backend 能力。
- 为后续 `Text Font 合同线` 的实现 plan 提供唯一 design truth，不在本设计里直接进入实现细节或越界收口其他剩余 gap。

## 背景

- 当前 `picoui_text_set_font()` 只把 `font` 指针存入 PicoUI `widget` 与 backend wrapper。
...
- 不把 `picoui_font` 改成 handle 型 API，也不把现有 public API 重写成 token-only 接口。
...
- `struct picoui_font { family, size }` 是描述值，不是 backend font 资源句柄。
- 调用方通过 `family` 与 `size` 表达“希望使用什么字体”，而不是把一个可长期借用、可被原样回传或必须按身份保存的 backend 对象交给 PicoUI。
- `picoui_text_set_font()` 的成功语义应是：
  - PicoUI 接收这份字体描述。
  - backend 依据该描述解析并绑定真实字体资源。
  - `text` 的真实渲染对象同步完成字体更新与必要的 reflow。
- PicoUI 不承诺 `font` 指针身份保持，不承诺 backend 内部真实字体对象与输入指针一一对应。

### ownership 与生命周期

- 调用方拥有 `struct picoui_font` 描述值本身；它只是一份输入描述，不是 backend 真实资源所有权的 transfer 载体。
- backend 负责真实字体资源的解析、缓存、fallback 和 cleanup 策略。
- `text` widget 不应直接拥有并释放调用方传入的 `struct picoui_font`。
...
- `picoui_text_set_font()` 是否对“描述可接受但需 fallback”的情况返回成功，由后续实现 spec 固定；本设计只要求 fallback 语义显式、稳定、不可悬空。
...
- 优点：
  - 与现有 public API 最一致。
  - 最符合 `PicoUI` 是上层抽象、`LingDongGUI` 是真实 backend 的分层。
...
- 把 `picoui_font` 实际变成稳定资源句柄或 backend-specific 对象入口。
- 优点：
  - 资源身份、缓存和生命周期更容易说清。
- 缺点：
  - 与现有 `struct picoui_font { family, size }` 明显冲突。
...
- 继续暴露 `struct picoui_font { family, size }`。
- `picoui_text_set_font()` 对外只接受描述值语义。
- PicoUI 测试不再把“输入指针被 backend wrapper 原样保存”当成最终合同证据。

### backend 抽象层

- backend 接收 PicoUI font 描述，并负责把它解析为可供 `ldText/text_box` 使用的真实字体。
- backend 必须成为 runtime rebind 的唯一入口，不能要求 demo 或上层 widget 直接操作 `ldText` 内部字段。
- backend 还要隔离真实字体资源生命周期，避免 PicoUI public API 直接卷入 `arm_2d_font_t` ownership。
...
1. 调用方构造或传入 `struct picoui_font { family, size }`。
2. `picoui_text_set_font()` 接收描述值并更新 PicoUI widget 侧状态。
...
- unit：
  ...
  - backend 失败时不留下 PicoUI/backend/render 三层分裂状态。
- contract：
...
- `text | picoui_text_set_font()` 不再依赖 wrapper-only 证据。

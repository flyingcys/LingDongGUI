...
> 日期：2026-06-05  
> 输入文档：`docs/tinyui-serial/a-0.14/2026-06-05-tinyui-port-boundary-audit.md`

## 背景

TINYUI 已经把控件 public API 与大部分 widget backend 映射收进 `tinyui/`，但芯片、屏幕、输入、tick、OS、资源适配仍主要依赖 `LingDongGUI` / `ARM-2D` 的旧 porting 入口。开发者如果要适配 TINYUI 到 SDL 或真实屏幕，仍会接触 `ldConfig.*`、`arm_2d_disp_adapter_0.*`、`Disp0_DrawBitmap()`、`LD_CFG_*`、`__DISP0_CFG_*`。

这和 TINYUI 目标冲突。TINYUI 应让开发者只面对 TINYUI 自己的 port 合同，而不是理解 LingDongGUI 与 ARM-2D 的存在。

## 目标

1. 建立 TINYUI 自己的 `port` 命名和目录，参考 LVGL 的 `lv_port_*` 心智。
2. 首批只做 SDL port 最小闭环：display、input、tick、OS。
3. 新 SDL 适配落在 `tinyui/port/sdl/`，不再挂到 `examples/sdl` 后面。
4. 不新增 `platform/ldgui`、`port/ldgui` 或等价目录；LingDongGUI 映射继续归属 `tinyui/src/backend/ldgui/`。
5. 保持 TINYUI public header 不泄漏 `ld*`、`arm_2d_*`、`SIGNAL_*`。
6. 保持现有 TINYUI unit/runtime/mapping/visible gate 可继续运行。
...
5. 不删除 `examples/sdl` 旧入口；本轮只是不再把新增 TINYUI SDL port 挂到那里。
...
`tinyui/src/display/`、`tinyui/src/indev/`、`tinyui/src/tick/`、`tinyui/src/osal/` 是 TINYUI core contract 和默认状态实现。`tinyui/port/sdl/` 是 SDL host 端口实现。`tinyui/src/backend/ldgui/` 只负责把 TINYUI app/widget 与 `src/display`、`src/indev`、`src/tick`、`src/osal` 状态映射到底层 LingDongGUI/ARM-2D。
...
这些默认值保留现有 SDL runtime 行为，但变成 TINYUI port 状态，而不是散在 `backend_app.c` 的宏。
...
LingDongGUI backend 后续应从 TINYUI input state 读取 pointer，再映射到 `ldCfgTouchSetPoint()` 或等价内部桥；SDL port 不直接调用 `ldCfgTouchSetPoint()`。
...
SDL port 只适配 TINYUI port 合同，不承载控件映射、不写 fake layout、不输出 backend mapping marker。
...
`tinyui_port_sdl_attach()` 的职责：

1. 设置 display 默认 config。
2. 设置 tick source。
3. 设置 delay callback。
4. 后续由 runtime event pump 把 SDL event 转成 `tinyui_input_push_pointer()` / `tinyui_input_push_key()`。

## Backend 边界

`tinyui/src/backend/ldgui/` 继续是 LingDongGUI/ARM-2D 唯一私有映射位置。允许它 include `ldConfig.h`、`ldGui.h`、`arm_2d.h`。不允许把这层拆成 `port/ldgui`。

首批 backend 调整原则：

1. `backend_app.c` 创建 window/texture/tile 时从 `tinyui_display_get_config()` 读取尺寸。
2. SDL event pump 改为调用 TINYUI input API。
3. LingDongGUI input bridge 再从 TINYUI input state 推到底层。
4. timer pump 使用 `tinyui_tick_get()`，不直接散落 SDL tick。
5. delay 使用 `tinyui_os_delay()`，不直接散落 SDL delay。

## 测试要求

新增 focused unit test：

- `tests/tinyui/unit/test_tinyui_port_display.c`
- `tests/tinyui/unit/test_tinyui_port_input.c`
- `tests/tinyui/unit/test_tinyui_port_tick_os.c`
- `tests/tinyui/unit/test_tinyui_port_sdl.c`

新增或扩展 contract：

- `tests/tinyui/contract/check_tinyui_public_api.py` 继续防止 public header 泄漏 `ld*`、`arm_2d_*`、`SIGNAL_*`。
...
若本地使用 `build/tinyui-runtime`，对应命令改为同一 build dir。
...
- TINYUI port header/core 状态。
- SDL attach 入口在 `tinyui/port/sdl/`。
...
后续单独设计，不混入 P1。目标是 `tinyui/port/mh2103c/` + `tinyui/docs/porting_guide.md`。

## 成功标准

1. 新增 TINYUI port API 不泄漏 LingDongGUI/ARM-2D 类型或名字。
2. SDL port 位于 `tinyui/port/sdl/`。
3. 新增 TINYUI SDL port 不依赖 `examples/sdl` 作为承载路径。
4. `backend_app.c` 的尺寸、pointer、tick、delay 不再直接以 SDL/LingDongGUI 旧宏为唯一源。
5. 现有 TINYUI app/widget API 不破坏。

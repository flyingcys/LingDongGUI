...
> 目标：在尽量不大改 `LingDongGUI` 内核的前提下，封装一个新的上层 API `TINYUI`，统一用户编程入口，隐藏 `LingDongGUI` 与 `ARM-2D` 细节，降低学习成本。
...
因此本轮不继续直接扩展单个控件，而是在 `LingDongGUI` 之上新增一个上层抽象：`TINYUI`。

`TINYUI` 的第一阶段目标：

1. 用户只使用统一的 `tinyui_*` API
...
`TINYUI` 是 `LingDongGUI` 上层抽象，不是新 GUI 内核。

优先级固定为：

1. TINYUI 统一 public API
2. TINYUI backend 做适配
...
`TINYUI` 作为上层抽象，`LingDongGUI` 作为当前唯一 backend/runtime。
...
- `ARM-2D`：更底层实现，不暴露给 `TINYUI` 用户

### 4.3 关键边界

#### public 层禁止泄漏

所有 `tinyui/include/tinyui/*.h` 中：
...
只有 `tinyui/src/backend/ldgui/` 允许直接碰：
...
```c
struct tinyui_button *btn = tinyui_button_create(parent, "ok_button");
tinyui_widget_set_size((struct tinyui_widget *)btn, 80, 32);
tinyui_button_set_text(btn, "OK");
tinyui_button_set_on_clicked(btn, on_ok_clicked, user_data);
```
...
struct tinyui_button *btn =
    tinyui_button_create_with_props(parent, &props);
```
...
- `tinyui_checkbox_set_text`
- `tinyui_checkbox_set_checked`
- `tinyui_checkbox_is_checked`
- `tinyui_checkbox_set_on_toggled`
...
- `tinyui_slider_set_value`
- `tinyui_slider_get_value`
- `tinyui_slider_set_range`
- `tinyui_slider_set_on_value_changed`
...
第一阶段 `part/state` 主要是 **TINYUI 统一语义层**，不强求 `LingDongGUI` 立即变成统一样式引擎。
...
用户不直接碰 `SIGNAL_*`，TINYUI 对外统一成 callback 风格。
...
```c
typedef void (*tinyui_event_cb)(struct tinyui_widget *widget,
                                void *user_data);
```
...
- `tinyui_label_set_font`
- `tinyui_text_set_font`
...
优先在 TINYUI 层完成统一与适配，不直接改 LingDongGUI 内核。

### 12.2 允许的最小补洞

只有当 TINYUI 确实依赖某个缺失能力时，才允许对 `LingDongGUI` 做最小增补，例如：
...
- 为 TINYUI 重写 `LingDongGUI` renderer
- 为 TINYUI 大规模改造 `LingDongGUI` 控件架构
- 为 TINYUI 引入超大范围样式系统翻修
...
1. 新用户只看 TINYUI 文档与 demo，即可完成：
   - 建页面
   - 放控件
   - 用 flex/grid 布局
   - 收事件
   - 应用 theme
2. TINYUI demo 源码不泄漏 `ld*` / `arm_2d_*`
3. public header 不泄漏 `LingDongGUI` 与 `ARM-2D` 类型
4. Linux 风格命名规则在 TINYUI public API 中保持一致
...
| `image` 对象映射与 source 绑定 | 已完成 | `tinyui_image_source` 已绑定到底层 `ldImage` |
...
`TINYUI` 第一阶段的正确定位，不是重写 `LingDongGUI`，也不是再造一个完整 `LVGL`，而是：
...
这套设计如果按边界执行，`TINYUI` 将更像一个稳定的应用层框架入口，而不是简单的 `ld*` 换前缀包装壳。
...
> 文档口径修正：本节之后若出现“已完成收敛”“runtime 已含启动级 smoke”“文档与代码已对齐”之类结论，均应以后续实际代码与测试真相为准。按当前主线核对，`tests/tinyui/runtime/check_tinyui_runtime.py` 虽然已经会构建并逐个启动 6 个 demo，但它使用独立 build tree `build/tinyui-runtime`，且 TINYUI demo 的构建链仍经由根入口默认开启的 SDL 子树，不应被表述为“已完全脱离 SDL、已全部闭环”。
...
当前状态是：`TINYUI` 的目录骨架、public API 基础形态、contract 检查和最小编译闭环已经落地；但“真正完成抽象层并映射到 LingDongGUI 行为”这件事仍未闭环，核心差距集中在 backend 实映射、事件链路、theme 真实下发和 demo 可运行闭环。

### 17.2 已完成项（可确认）

1. 目录结构与主要文件已落地（`tinyui/include`、`tinyui/src`、`tinyui/demo`、`tinyui/docs`、`tests/tinyui`）。
2. public header 边界已建立：`tinyui/include/tinyui/*.h` 未泄漏 `ld*` / `arm_2d_*` / `SIGNAL_*`。
3. 第一阶段声明的 8 个基础控件 + `flex/grid` + `theme v0` 基础 API 已有最小接口外形。
4. TINYUI 测试分层已落地到 `tests/tinyui/{unit,contract,runtime}`，并可通过 `ctest -L tinyui` 跑通。
5. `README.md` 与教程已加入 TINYUI 入口，具备最小文档导航。

### 17.3 部分完成项（有接口，但未形成真实能力）

1. backend 当前是“占位实现”为主：`tinyui/src/backend/ldgui/*.c` 主要做空校验或本地结构体赋值，尚未完成对 `LingDongGUI` 控件、布局、事件系统的真实映射。
2. 事件 contract 未闭环：`switch/checkbox/slider` 回调主要由 setter 主动触发，非来自底层事件；`button` 缺少 `tinyui_button_set_on_clicked` 对外接口。
...
1. 通用 widget API 存在缺口：设计中的 `tinyui_widget_set_text`、`tinyui_widget_set_style_class`、`tinyui_widget_set_user_data` 未落地。
2. style 快捷接口未落地：`tinyui_widget_set_bg_color`、`tinyui_widget_set_text_color`、`tinyui_widget_set_border_color`、`tinyui_widget_set_radius`、`tinyui_widget_set_padding` 未落地。
3. 主题语义缺口：`PICOUI_STATE_*` / `PICOUI_PART_*` 词汇未在 public API 中建模。
4. 资源抽象缺口：`struct tinyui_font`、`tinyui_label_set_font`、`tinyui_text_set_font` 未落地（当前仅有 `image_source` 最小占位）。
5. 验收标准第 1 条（新用户仅靠 TINYUI 文档与 demo 即可完成完整页面搭建）当前证据不足。
...
1. 为 6 个 TINYUI demo 建立统一可执行目标（至少可批量 build）。
2. 增加 demo 运行级检查（不仅构建成功，还要最小运行返回）。
3. 在 `tinyui/docs` 增加“当前已实现 vs 规划能力”矩阵，明确已知边界，避免过度承诺。
...
1. **通用 widget API 缺口已补齐**
   - 已补：`tinyui_widget_set_text`、`tinyui_widget_set_style_class`、`tinyui_widget_set_user_data`
   - 已补：`tinyui_widget_set_bg_color`、`tinyui_widget_set_text_color`、`tinyui_widget_set_border_color`、`tinyui_widget_set_radius`、`tinyui_widget_set_padding`
2. **button 事件接口已补齐**
   - 已补：`tinyui_button_set_on_clicked`
3. **theme 统一语义词汇已落地**
   - 已补：最小 `PICOUI_STATE_*`、`PICOUI_PART_*` public 枚举
4. **font 资源抽象已补齐最小 contract**
   - 已补：`struct tinyui_font`、`tinyui_label_set_font`、`tinyui_text_set_font`
5. **backend 从纯 no-op 升级为“可承接状态”的实现**
   ...
   - 新增 clicked 统一触发入口 `tinyui_backend_emit_clicked`
6. **demo 可执行闭环已补齐**
   - 6 个 TINYUI demo 均有 `main`，并在 SDL CMake 下注册独立 target
   - runtime 检查从单 target 扩展为批量构建 6 个 demo target

### 21.2 验证证据（本轮）

1. `ctest --test-dir build -L tinyui --output-on-failure`：通过（7/7）
2. `tests/tinyui/runtime/check_tinyui_runtime.py`：已覆盖 6 个 demo target 构建
3. `tests/tinyui/contract/check_tinyui_demo_boundary.py`：通过
...
3. `tinyui_theme_create()` 已从全零默认值切换为 theme v0 默认 token。
4. `tests/tinyui/unit/test_tinyui_theme.c` 已补充默认 token 非零与 `set_color` / `set_metric` 覆盖行为验证。
...
> 本节用于回答“`docs/superpowers/specs/2026-05-26-tinyui-abstraction-layer-design.md` 是否已全部开发完成”。
>
> 文档口径修正：以下 `22.x` 内容属于 2026-05-26 当日的阶段性 review 复盘快照，不再代表当前最终完成判定。当前主线真相以第 15.1 节能力矩阵、`docs/tinyui-serial/A-线计划索引.md` 的 `A7` 收口状态，以及最新 Stage G 门禁复验结果为准。
>
...
- 代码与头文件：`tinyui/include/tinyui/*.h`、`tinyui/src/**/*`、`tinyui/demo/*/main.c`、`examples/sdl/CMakeLists.txt`。
- 测试：`tests/tinyui/{unit,contract,runtime}`。
- 运行证据：`rtk ctest --test-dir build -L tinyui --output-on-failure`（本轮复验 8/8 通过）。
...
1. 6 个 TINYUI demo 构建目标齐全，并已接入 runtime 构建检查（`tinyui_*_demo` 全部纳入）。
2. `PICOUI_STATE_*` / `PICOUI_PART_*` 已在 public API 建模。
3. `tinyui_button_set_on_clicked`、通用 widget 文本/样式/用户数据与常用样式快捷接口已补齐。
4. `struct tinyui_font`、`tinyui_label_set_font`、`tinyui_text_set_font` 已提供最小 contract。
5. `ctest -L tinyui` 当前链路可运行，但其 runtime 子项依赖独立 build tree 与 SDL demo 构建链，文档不能把它简化成“纯 TINYUI、自身完全独立”的闭环。
...
1. **checkbox 文本 API 与文档不一致**
   - 第 6.5 节声明了 `tinyui_checkbox_set_text`，当前头文件/实现未提供该接口。
2. **事件词汇未全落地**
...
4. **font / style_class / user_data 的 backend 下发链路不完整**
   - `label/text` 的字体设置目前主要停留在 TINYUI 本地字段写入，未完整串联到通用 backend 同步路径。
...
7. **测试与入口文档仍有表述偏差**
   ...
   - `tinyui/docs/demo_guide.md` 仍把 `examples/sdl` 子目录 configure 描述为主路径，这与当前推荐入口不一致。

### 22.5 下一步工作（按优先级）

#### P0：先补“文档直连缺口 + 事件闭环”

1. 补 `tinyui_checkbox_set_text`（header + impl + unit test + contract 用例）。
2. 增补 `pressed/released` 最小 public API（建议先从 button 起步），并补 backend signal 与测试。
3. 在文档中明确：哪些事件当前是“setter 触发语义”，哪些是“底层上送语义”。

#### P1：补“行为闭环证据”

1. 为 `tinyui_widget_set_flex_new_track`、`tinyui_widget_set_ignore_layout`、`tinyui_grid_set_align` 增加行为级断言。
...
3. 在 `tinyui/docs` 增加“已实现能力矩阵（已完成 / 部分完成 / 规划）”防止过度承诺。
...
3. `ctest -L tinyui` 全通过，且 runtime 至少包含最小启动级 smoke。
...
1. **`tinyui_checkbox_set_text` 已落地**
   - 已补 public API：`tinyui/include/tinyui/checkbox.h`
   - 已补实现：`tinyui/src/widgets/checkbox.c`
   - 已补 props 文本初始化：`tinyui_checkbox_create_with_props` 支持 `text`
2. **`pressed/released` 最小事件 contract 已落地（button）**
   - 已补 public API：`tinyui_button_set_on_pressed`、`tinyui_button_set_on_released`
   - 已补 backend signal：`PICOUI_BACKEND_SIGNAL_PRESSED`、`PICOUI_BACKEND_SIGNAL_RELEASED`
   - 已补 dispatch 入口：`tinyui_backend_widget_dispatch_event(...)`
3. **单测闭环已补**
   - 新增：`tests/tinyui/unit/test_tinyui_button_events.c`
   - 既有：`tests/tinyui/unit/test_tinyui_widgets.c` 已补 checkbox 文本路径断言
   - 测试接线：`tests/tinyui/CMakeLists.txt` 新增 `test_tinyui_button_events`

### 23.2 本轮验证证据

1. `rtk cmake --build build --target test_tinyui_smoke test_tinyui_theme test_tinyui_widgets test_tinyui_button_events test_tinyui_layout tinyui_hello_world_demo tinyui_basic_widgets_demo tinyui_layout_flex_demo tinyui_layout_grid_demo tinyui_theme_showcase_demo tinyui_settings_panel_demo`：通过。
2. `rtk ctest --test-dir build -L tinyui --output-on-failure`：通过（8/8）。
3. GitNexus `detect_changes(scope=all)`：`risk_level=low`，`affected_count=0`（当前改动范围未扩散到已建模流程）。

### 23.3 更新后的剩余工作（进入下一轮）

> 历史快照（已过时）：以下条目是 2026-05-26 当次 review 结束时，准备进入下一轮时记录的剩余工作清单，对应当时的 `22.4` 阻塞项；它们用于解释那一刻为什么尚不能判定“全部开发完成”，但不再代表当前 A 线 final closeout 的现态 blocker。`A7` 后续收口、门禁复验与索引回写完成后，当前最终状态应以第 `15.1` 节能力矩阵、`docs/tinyui-serial/A-线计划索引.md` 的 `A7` 收口状态，以及最新 Stage G 门禁结果为准。
>
> 当时记录的剩余工作如下：
>
...
1. **layout 行为级证据已补强**
   - `tests/tinyui/unit/test_tinyui_layout.c` 已新增对 `tinyui_widget_set_flex_new_track`、`tinyui_widget_set_ignore_layout`、`tinyui_grid_set_align` 的行为断言。
   - 断言同时覆盖 TINYUI 前端状态与 backend 状态承接字段，减少“仅返回码通过”的弱证据。
2. **`basic_widgets` demo 覆盖已对齐**
   - `tinyui/demo/basic_widgets/main.c` 在原有 `switch/checkbox/slider` 之外，新增了 `button/text/image` 的创建与最小设置调用。
   - 保持 demo 边界约束：未泄漏 `ld*` / `arm_2d_*` / `SIGNAL_*`。
3. **验证结论**
   - `rtk cmake --build build --target test_tinyui_layout tinyui_basic_widgets_demo`：通过。
   - `rtk ctest --test-dir build -L tinyui --output-on-failure`：通过（8/8）。
   - GitNexus `detect_changes(scope=all)`：`risk_level=low`，`affected_count=0`。

### 23.5 收敛更新（2026-05-26，继续并行推进）

1. **state/part 最小应用语义已落地**
   - 新增 public API：`tinyui_theme_apply_to_widget(theme, widget, part, state)`。
...
2. **font/style_class/user_data backend 同步链路已闭环**
   - `tinyui_widget_set_style_class`、`tinyui_widget_set_user_data` 已在 core setter 内同步 backend。
   - `tinyui_label_set_font`、`tinyui_text_set_font` 已同步 backend font 字段。
   - `test_tinyui_widgets` 已补 backend 字段断言（style_class/user_data/font）。
3. **runtime 最小启动级 smoke 已落地**
   - `tests/tinyui/runtime/check_tinyui_runtime.py` 由“仅构建”升级为“构建 + 逐个启动 6 个 demo + 退出码校验 + 超时保护”。
4. **复验证据**
   - `rtk cmake --build build --target test_tinyui_smoke test_tinyui_theme test_tinyui_widgets test_tinyui_button_events test_tinyui_layout tinyui_hello_world_demo tinyui_basic_widgets_demo tinyui_layout_flex_demo tinyui_layout_grid_demo tinyui_theme_showcase_demo tinyui_settings_panel_demo`：通过。
   - `rtk ctest --test-dir build -L tinyui --output-on-failure`：通过（8/8，runtime 启动级 smoke 纳入通过）。
   - GitNexus `detect_changes(scope=all)`：`risk_level=low`，`affected_count=0`。
...
1. **runtime 的真实含义**
   ...
   - 但 runtime 仍通过 `build/tinyui-runtime` 独立 build tree 执行，不是复用常规 `build/`。
2. **入口的真实含义**
   - 当前推荐 configure/build 入口是仓库根 `CMakeLists.txt`。
   - 但由于 `LD_BUILD_SDL_DEMO=ON` 默认开启，根入口默认仍会进入 SDL 子树，不能写成“根测试入口已与 SDL 解耦”。
3. **分层的真实含义**
   - `tests/tinyui/runtime` 与 `ctest -L runtime` 说明 runtime 已分层。
   - 这只代表“可以按 label 单独执行”，不代表 runtime 默认不注册或默认被排除。
4. **demo 文档的真实含义**
   - `tinyui/docs/demo_guide.md` 当前仍保留 `cmake -S examples/sdl -B examples/sdl/build` 作为主要示例。
   - 该文件现有用户脏改未提交，因此本轮只在上层文档中提示偏差，不直接修改该文件。

### 23.6 当前判定（对照 22.6 门禁）

> 历史快照（已过时）：
> 本小节记录的是 `2026-05-26` 当次 review 对照 `22.6` 门禁时的阶段性判定，不再代表当前 `A线` final closeout 的现态结论。当前应以 `15.1` 能力矩阵、`docs/tinyui-serial/A-线计划索引.md` 的 `A7` 收口状态，以及最新 `Stage G` fresh gate 为准。

按第 22.6 的 4 条门禁逐项核对：

1. 第 6/7/9/10/11 节中的主要 contract 缺口已有代码与测试证据。  
2. 关键行为项已不再只依赖返回码，runtime 也已包含最小启动级 smoke。  
3. 但 `ctest -L tinyui` 的 runtime 仍依赖独立 build tree 与 SDL demo 构建链。  
...
> 本节是当前 `TINYUI` 主线的纠偏真相源，用于回答“为什么现在能弹窗但 UI 效果仍不对，以及后续应该按什么方向继续开发”。

### 24.1 当前问题不是“SDL 不支持 TINYUI”

当前问题更准确应表述为：

- `SDL` 作为宿主层本身没有问题；
- `TINYUI` 当前已经能构建、能启动、能在 SDL 窗口内输出画面；
- 当前主输出链已经是 `ldGuiFrameStart() -> ldMsgProcess() -> ldGuiDraw() -> ldGuiFrameComplete()` 的真实 `LingDongGUI` 绘制路径；
- `tinyui/src/backend/ldgui/backend_app.c` 仍然存在，但当前口径应明确为 `temporary smoke path` / host harness：它还负责 SDL host、capture、marker、placeholder 等过渡职责，但不再承担正式 fake renderer 主输出职责；
- 因此，“窗口起来了”依然不等于“单靠 runtime/capture 就能证明最终 UI 完成”；真正成立的是：`TINYUI -> LingDongGUI` 真实 backend 主线已经建起来，而 `runtime/capture` 继续只承担 smoke / 回归证据角色。
...
2. `tinyui/demo/*/main.c` 通过补 `set_size()`、`set_pos()` 或其他人工摆位来配合 fake renderer。
...
4. SDL 只负责宿主显示，不再为 `TINYUI` 单独维护一套视觉系统。

### 24.4 A线当前口径

从本节开始，`TINYUI` 的当前主线统一记为 **A线**，其正式目标不是“继续把 fake renderer 修漂亮”，而是：

- **A线目标**：把 `TINYUI` 从“临时可视 smoke”纠偏成“真实 `TINYUI -> LingDongGUI` backend 映射主线”。

### 24.5 A线当前禁止项

后续开发中，以下行为默认禁止：

1. 继续在 `tinyui/src/backend/ldgui/backend_app.c` 中新增固定坐标、固定尺寸、手工 line/rect/circle 控件画法。
2. 继续在 `tinyui/demo/*/main.c` 里增加为了“看起来像 UI”而写的硬编码尺寸/位置补丁。
3. 继续把 “能弹窗 / 有 capture / 画面非空” 当成 “backend 映射已完成”。
4. 借着修 demo 外观，顺手把 `LingDongGUI` 渲染链重写成 `TINYUI` 专属实现。

### 24.6 A线正式入口

后续关于 `TINYUI` 当前该做什么、已做到哪一阶段、哪些不能做，统一以：

- `docs/superpowers/specs/2026-05-26-tinyui-abstraction-layer-design.md`
- `docs/superpowers/plans/2026-05-26-tinyui-abstraction-layer-implementation.md`
- `docs/tinyui-serial/A-线计划索引.md`

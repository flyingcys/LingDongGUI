# TINYUI Text Font 合同线 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 `text | tinyui_text_set_font()` 从 wrapper-only 存储提升为真实 backend 字体解析、runtime rebind、reflow 与 cleanup 合同，同时保持现有 `struct tinyui_font { family, size }` public API 形状不变。

**Architecture:** 方案保持 `tinyui_font` 为描述值，由 TINYUI backend 负责解析 `family/size` 到真实 `arm_2d_font_t` 资源，再通过稳定的 `ldText/text_box` 更新路径完成 runtime 字体切换。实现必须显式隔离调用方描述值与 backend 内部字体资源生命周期，避免继续依赖“覆写 `ptFont` 指针 + `ldText_depose()` 条件释放”的隐式 ownership。

**Tech Stack:** C, CMake, TINYUI abstraction layer, LingDongGUI `ldText`, Arm-2D `text_box`, GitNexus impact analysis, `ctest`
...
- Modify: `tinyui/src/widgets/text.c`
  - 保持 `tinyui_text_set_font()` 的 public 入口，但把成功语义从 wrapper-only 存储升级为真实 backend update。
...
- Modify: `tinyui/src/backend/ldgui/backend_label.c`
  - 若当前通用 `tinyui_backend_widget_set_font()` 定义在此，需把 `text` 与 `label` 的字体路径边界拆清，避免复用错误语义。
- Modify: `tinyui/src/backend/ldgui/backend_widget.h`
...
- Modify: `tests/tinyui/unit/test_tinyui_widgets.c`
  - 增补 TINYUI text font 的 fail-first 单测，覆盖 `NULL` fallback、runtime rebind、backend failure 不分裂等合同。
...
**Files:**
- Modify: `tests/tinyui/unit/test_tinyui_widgets.c`
- Reference: `docs/superpowers/specs/2026-05-30-tinyui-text-font-contract-line-design.md`
- Reference: `tinyui/src/widgets/text.c`
- Reference: `tinyui/src/backend/ldgui/backend_text.c`
...
```bash
printf '%s\n' \
  'impact target: tinyui_text_set_font' \
  'impact target: tinyui_backend_widget_set_font' \
...
在 `tests/tinyui/unit/test_tinyui_widgets.c` 新增以下测试函数骨架，并把它们挂到主测试入口：
...
static void test_text_font_null_falls_back_to_default_contract(void)
{
    ...
    struct tinyui_text *text = tinyui_text_create(win, "body");
    struct tinyui_backend_widget *backend = text->widget.backend_widget;
    ldText_t *ld_text = (ldText_t *)backend->ld_widget;

    assert(text != 0);
    assert(ld_text != 0);
    assert(tinyui_text_set_font(text, NULL) == 0);
...
建议做法：在测试里替换或注入一个 backend font 解析失败路径，确保 `tinyui_text_set_font()` 返回失败时旧真实字体状态仍保持稳定。
...
static void test_text_font_backend_failure_does_not_split_state(void)
{
    ...
    assert(tinyui_text_set_font(text, &bad_font) == -1);
...
```bash
cmake --build build --target test_tinyui_widgets
ctest --test-dir build -R '^test_tinyui_widgets$' --output-on-failure
```

Expected: `test_tinyui_widgets` 失败，且失败点来自新加的 `text font` 合同断言，而不是编译错误或无关测试。

- [ ] **Step 7: 提交 fail-first 测试**

```bash
git add tests/tinyui/unit/test_tinyui_widgets.c
git commit -m "test: add tinyui text font contract cases"
```
...
在 `tinyui/src/backend/ldgui/backend_widget.h` 增加最小接口声明，例如：

```c
int tinyui_backend_text_apply_font(struct tinyui_backend_widget *widget,
...
在 `tinyui/src/backend/ldgui/backend_text.c` 添加最小解析函数框架：

```c
static arm_2d_font_t *tinyui_backend_text_resolve_font(const struct tinyui_font *font)
{
    (void)font;
...
在 `tinyui_backend_text_apply_font(...)` 里最少做到：

```c
arm_2d_font_t *resolved = tinyui_backend_text_resolve_font(font);
if (resolved == NULL) {
    return -1;
}
```
...
- [ ] **Step 4: 修改 `tinyui_text_set_font()` 只在 backend 成功后提交 public 状态**
...
int tinyui_text_set_font(struct tinyui_text *text, const struct tinyui_font *font)
{
    ...
    if (tinyui_backend_text_apply_font(text->widget.backend_widget, font, 1) != 0) {
        return -1;
    }
...
```bash
git add tinyui/src/backend/ldgui/backend_text.c tinyui/src/backend/ldgui/backend_widget.h tinyui/src/backend/ldgui/backend_label.c tinyui/src/widgets/text.c tests/tinyui/unit/test_tinyui_widgets.c
...
在 `tinyui/src/backend/ldgui/backend_text.c` 中，把 `resolved` font 通过 `ldTextSetFont(...)` 应用到真实 `ldText`，并只在成功后提交 backend wrapper 状态。
...
```bash
git add src/gui/ldText.h src/gui/ldText.c tinyui/src/backend/ldgui/backend_text.c tests/tinyui/unit/test_tinyui_widgets.c
...
在 `tinyui/src/backend/ldgui/backend_text.c` 或测试辅助区加入仅测试可用的 failure 注入点，例如：
...
Expected: `test_tinyui_widgets` 通过，且新增的三条 `text font` 合同用例全部为绿色。
...
```bash
printf '%s\n' \
  'recheck impact target: tinyui_text_set_font' \
...
Expected: 不要求 `text font` 由整组标签测试单独证明可见视觉效果，但要确认此次改动未破坏既有 TINYUI 测试面。

- [ ] **Step 4: 更新 G 线 capability gap matrix**

把 `text | tinyui_text_set_font()` 行更新为类似结论：

```md
| text | `tinyui_text_set_font()` | backend text font resolve + `ldText` runtime rebind + reflow | support | unit / contract | `Text Font 合同线` 已收口为描述值 font -> backend 解析 -> 真实 `ldText/text_box` 更新合同；不承诺 public 指针身份保持 |
```

- [ ] **Step 5: 更新索引与剩余缺口入口**

在 `docs/tinyui-serial/G-线计划索引.md` 和 `docs/tinyui-serial/G-线剩余缺口合同决策入口.md` 中：
...
- Type consistency:
  - 统一使用 `struct tinyui_font { family, size }`、`tinyui_text_set_font()`、`ldTextSetFont(...)`、`ptFont`、`ownsFont` 这些同一语义标识；若实现中最终命名微调，后续 review 必须同步检查所有文档与测试。

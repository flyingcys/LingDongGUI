# TINYUI Image 语义线 Implementation Plan
...
**Architecture:** 这条线不新增 `image` 真实视觉或 disabled backend；它只冻结 public contract 与测试口径，避免把不存在的 `ldImage` style/state 承接点误表述成“差一点 support”。实现方式是以现有 `tinyui_widget_*` 与 backend style dispatch 的真实行为为锚点，补齐最小 contract/unit 证据，并把矩阵、索引、决策入口统一到同一设计结论上。

**Tech Stack:** C, CMake, TINYUI abstraction layer, LingDongGUI `ldImage`, GitNexus impact analysis, Python contract checks, `ctest`
...
**Files:**
- Modify: `tests/tinyui/unit/test_tinyui_widgets.c`
- Reference: `docs/superpowers/specs/2026-05-30-tinyui-image-semantics-line-design.md`
- Reference: `tinyui/src/backend/ldgui/backend_style_apply.c`
...
```bash
printf '%s\n' \
  'impact target: tinyui_backend_widget_apply_style' \
  'impact target: tinyui_widget_set_enabled'
```
...
在 `tests/tinyui/unit/test_tinyui_widgets.c` 增加以下测试函数骨架，并挂到主测试入口：
...
static void test_image_style_class_and_user_data_are_metadata_only_contract(void)
{
    ...
    struct tinyui_image *image = tinyui_image_create(win);
    ...
    assert(tinyui_widget_set_user_data(&image->widget, user_data) == 0);
...
static void test_image_theme_style_parts_remain_explicitly_rejected(void)
{
    ...
    struct tinyui_window *win = tinyui_window_create(tinyui_app_root(app), "root");
    struct tinyui_image *image = tinyui_image_create(win);
    struct tinyui_theme_token token = {
        .bg = 0xff112233u,
        .fg = 0xff445566u,
        .border = 0xff778899u,
        .radius = 6,
    ...
    assert(image != 0);
    assert(tinyui_theme_apply_to_widget(&image->widget, PICOUI_PART_MAIN, &token) == -1);
    assert(tinyui_theme_apply_to_widget(&image->widget, PICOUI_PART_TEXT, &token) == -1);
...
static void test_image_enabled_remains_rejected_contract(void)
{
    ...
    assert(image != 0);
    assert(tinyui_widget_set_enabled(&image->widget, 0) == -1);
...
Expected: `test_tinyui_widgets` 失败，且失败点来自新加的 `image` 语义合同断言，而不是编译错误或无关测试。

- [ ] **Step 7: 提交 fail-first 测试**

```bash
git add tests/tinyui/unit/test_tinyui_widgets.c
git commit -m "test: add tinyui image semantics cases"
```
...
若 `image` 当前某些 part 没有稳定返回 `-1`，在 `tinyui/src/backend/ldgui/backend_style_apply.c` 把 `PICOUI_BACKEND_WIDGET_IMAGE` 的 reject 逻辑显式收紧成类似：
...
若 `tinyui_widget_set_enabled(&image->widget, ...)` 当前不是稳定 `-1`，在 `tinyui/src/core/widget.c` 把 `image` 保持在 reject 路径。例如：
...
如果 `style_class / user_data` 测试已经能证明 TINYUI 与 backend wrapper 存储一致，则不要额外修改 `ldImage` 或 backend create 路径；本任务的正确结果通常是“测试转绿，代码不动”。
...
Expected: 新增 `image` 语义用例全部通过，且不引入其他 `tinyui widgets` 回归。

- [ ] **Step 5: 提交 image 语义最小收口**

```bash
git add tinyui/src/backend/ldgui/backend_style_apply.c tinyui/src/core/widget.c tests/tinyui/unit/test_tinyui_widgets.c
git commit -m "feat: freeze tinyui image semantics contracts"
```
...
```md
| image | `tinyui_widget_set_style_class()`、`tinyui_widget_set_user_data()` | 无真实 `ldImage` 消费链；当前只更新 TINYUI `widget` 与 backend wrapper 字段 | incomplete_contract | unit / contract | 当前只承认 metadata-only 存储语义 |
| image | `tinyui_widget_set_bg_color()`、`tinyui_widget_set_text_color()`、`tinyui_widget_set_border_color()`、`tinyui_widget_set_radius()` | `backend_style_apply` 对 `PICOUI_BACKEND_WIDGET_IMAGE` 显式拒绝 | reject | unit / contract | 不存在 image-style backend 承接点 |
| image | `tinyui_widget_set_padding()` | 无稳定 image content box / layout spacing 合同 | deferred | contract | 语义未冻结，暂不承诺 |
| image | `tinyui_widget_set_enabled()` | 无 image-specific disabled backend 语义 | reject | unit / contract | 不偷换成 `visible` / `opacity` / `selectable` |
```
...
git commit -m "docs: record tinyui image semantics line"
```
...
Expected: `image` 语义合同测试与现有 `tinyui` 测试全集全部通过。
...
```bash
git add tests/tinyui/unit/test_tinyui_widgets.c tinyui/src/backend/ldgui/backend_style_apply.c tinyui/src/core/widget.c docs/superpowers/specs/2026-05-29-tinyui-g-line-current-widget-capability-gap-matrix.md docs/tinyui-serial/G-线计划索引.md docs/tinyui-serial/G-线剩余缺口合同决策入口.md
git commit -m "feat: close tinyui image semantics line"
```

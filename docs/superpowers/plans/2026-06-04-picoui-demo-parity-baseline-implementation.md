# TINYUI Demo Parity Baseline Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 新增 `legacy-widget`、`layout`、`grid` 三个 TINYUI parity baseline demo，使其页面结构更接近老 SDL demo，便于双边截图对比定位 TINYUI 真缺口。

**Architecture:** 以老 demo 源码为真相源，在 `tinyui/demo` 下新增三个独立入口，全部只使用 `tinyui_*` public API。实现顺序先锁定 target 注册与 demo boundary，再逐页落 `legacy_widget_parity`、`layout_parity`、`grid_parity`，最后做编译与 contract 验证。

**Tech Stack:** C, CMake, TINYUI public API, SDL demo targets, Python contract checks

---

## File Structure

- Create: `tinyui/demo/legacy_widget_parity/main.c`
- Create: `tinyui/demo/layout_parity/main.c`
- Create: `tinyui/demo/grid_parity/main.c`
- Optional Create: `tinyui/demo/common/` 下少量纯 TINYUI helper
...
在 `tests/tinyui/contract/check_tinyui_demo_boundary.py` 新增源码级断言，检查以下 target 字符串存在：
...
按现有 `add_tinyui_demo(...)` 模式新增：

```cmake
add_tinyui_demo(tinyui_legacy_widget_parity_demo
    "${SDL_EXAMPLE_DIR}/../../tinyui/demo/legacy_widget_parity/main.c"
)
add_tinyui_demo(tinyui_layout_parity_demo
    "${SDL_EXAMPLE_DIR}/../../tinyui/demo/layout_parity/main.c"
)
add_tinyui_demo(tinyui_grid_parity_demo
    "${SDL_EXAMPLE_DIR}/../../tinyui/demo/grid_parity/main.c"
)
```
...
```bash
git add examples/sdl/CMakeLists.txt tests/tinyui/contract/check_tinyui_demo_boundary.py
git commit -m "test: lock tinyui parity demo targets"
```
...
Run: `rtk cmake --build build --target tinyui_legacy_widget_parity_demo tinyui_layout_parity_demo tinyui_grid_parity_demo`
...
```bash
git add tinyui/demo/legacy_widget_parity/main.c tinyui/demo/layout_parity/main.c tinyui/demo/grid_parity/main.c
git commit -m "feat: add tinyui parity demo skeletons"
```
...
- Optional Create: `tinyui/demo/common/*`
- Reference: `examples/common/demo/widget/uiWidgetLegacy.c`

- [ ] **Step 1: 先写失败断言，要求 demo 中出现关键 widget 构造痕迹**

在 `tests/tinyui/contract/check_tinyui_demo_boundary.py` 或同层新 contract 中增加源码级字符串断言，至少覆盖：
...
在 `tinyui/demo/legacy_widget_parity/main.c` 用 `tinyui_*` 组装：
...
Run: `rtk cmake --build build --target tinyui_legacy_widget_parity_demo`
...
- Optional Create/Modify: `tinyui/demo/common/*`
...
在 `tinyui/demo/layout_parity/main.c` 组织：
...
Run: `rtk cmake --build build --target tinyui_layout_parity_demo`
...
```bash
git add tinyui/demo/layout_parity/main.c tests/tinyui/contract/check_tinyui_demo_boundary.py
...
在 `tinyui/demo/grid_parity/main.c` 组织：
...
Run: `rtk cmake --build build --target tinyui_grid_parity_demo`
...
```bash
git add tinyui/demo/grid_parity/main.c tests/tinyui/contract/check_tinyui_demo_boundary.py
...
把新 demo 名称、目标、用途写进 `tinyui/docs/demo_guide.md`，说明它们是截图对比用 `parity baseline`，不是普通 showcase demo。
...
```bash
git add tinyui/docs/demo_guide.md
git commit -m "docs: register tinyui parity baseline demos"
```

...
**目标:** 把 TINYUI B 线已有的自动 visible correctness 结果，工程化为文档口径准确、CTest 可执行、mapping matrix 可维护、可选人工 artifact 可追溯的长期门禁。
...
**技术栈:** CMake、CTest、Python3、SDL2 host runtime、TINYUI runtime scripts、LingDongGUI、GitNexus、Markdown serial docs
...
- Modify: `docs/tinyui-serial/C-线计划索引.md`
...
- Optional Modify: `tests/tinyui/runtime/check_tinyui_backend_mapping.py`
...
- Optional Modify: `tinyui/demo/*/main.c`
- Optional Modify: `tinyui/src/backend/ldgui/backend_app.c`
- Modify: `docs/superpowers/specs/2026-05-26-tinyui-lingdonggui-test-architecture-design.md`
...
**文件:**
- Create: `tests/tinyui/runtime/check_tinyui_manual_window_artifact.py`
- Create: `docs/tinyui-serial/C-线人工窗口验收记录.md`
- Modify: `tinyui/docs/demo_guide.md`
- Modify: `docs/tinyui-serial/C-线计划索引.md`
...
git add docs/tinyui-serial/B-线计划索引.md \
        ...
        docs/superpowers/specs/2026-05-26-tinyui-lingdonggui-test-architecture-design.md \
        tinyui/docs/demo_guide.md
git commit -m "docs(tinyui): clarify c-line gate evidence"
```
...
```bash
sed -n '1,120p' tests/tinyui/CMakeLists.txt
...
- 在 `tests/tinyui/CMakeLists.txt` 新增 `check_tinyui_visible_ui`。
- 脚本为 `tests/tinyui/runtime/check_tinyui_visible_ui.py`。
- 参数固定为 `--all`。
- labels 至少为 `tinyui;runtime;visible`。
...
- `ctest -N` 能列出 `check_tinyui_visible_ui`。
- targeted CTest PASS。
...
- 影响范围符合 `tests/tinyui/CMakeLists.txt` 和 runtime gate wiring。
- 如果 CLI 不可用，改用 MCP `gitnexus_detect_changes(scope="all")` 并记录结果。

- [ ] **Step 7: 提交**

```bash
git add tests/tinyui/CMakeLists.txt \
        tests/tinyui/runtime/check_tinyui_visible_ui.py \
        docs/superpowers/specs/2026-05-26-tinyui-lingdonggui-test-architecture-design.md \
        docs/tinyui-serial/C-线计划索引.md
git commit -m "test(tinyui): wire visible gate into ctest"
```
...
- 在 `tests/tinyui/CMakeLists.txt` 新增 `check_tinyui_backend_mapping`。
- 脚本为 `tests/tinyui/runtime/check_tinyui_backend_mapping.py`。
- labels 至少为 `tinyui;runtime;backend;mapping`。
...
- `ctest -N` 能列出 `check_tinyui_backend_mapping`。
- targeted CTest PASS。
...
git add tests/tinyui/CMakeLists.txt \
        ...
        docs/superpowers/specs/2026-05-26-tinyui-lingdonggui-test-architecture-design.md \
        docs/tinyui-serial/C-线计划索引.md
git commit -m "test(tinyui): wire backend mapping gate into ctest"
```
...
```bash
git add docs/superpowers/specs/2026-05-26-tinyui-lingdonggui-test-architecture-design.md \
        docs/tinyui-serial/C-线计划索引.md \
        tinyui/docs/demo_guide.md
# 如果 Step 4 修改了现有 CI 配置，把对应 CI 文件显式加入 git add。
git commit -m "docs(tinyui): document c-line gate matrix"
```
...
**文件:**
- Modify: `tests/tinyui/runtime/check_tinyui_backend_mapping.py`
- Optional Modify: `tinyui/demo/hello_world/main.c`
- Optional Modify: `tinyui/demo/basic_widgets/main.c`
- Optional Modify: `tinyui/demo/layout_flex/main.c`
- Optional Modify: `tinyui/demo/layout_grid/main.c`
- Optional Modify: `tinyui/demo/theme_showcase/main.c`
- Optional Modify: `tinyui/demo/settings_panel/main.c`
- Optional Modify: `tinyui/src/backend/ldgui/backend_app.c`
- Modify: `docs/superpowers/specs/2026-05-26-tinyui-lingdonggui-test-architecture-design.md`
- Modify: `docs/tinyui-serial/C-线计划索引.md`
...
```text
gitnexus_impact(target="check_tinyui_backend_mapping.py", direction="upstream")
gitnexus_impact(target="tinyui_backend_app_run", direction="upstream")
```
...
```bash
git add tests/tinyui/runtime/check_tinyui_backend_mapping.py \
        ...
        docs/superpowers/specs/2026-05-26-tinyui-lingdonggui-test-architecture-design.md \
        docs/tinyui-serial/C-线计划索引.md
git commit -m "test(tinyui): expand backend mapping matrix"
```
...
- 支持 `--artifact-dir artifacts/tinyui/manual-window`。
- 输出 artifact 路径。
- 若环境无窗口或 demo 无法运行，失败信息必须说明这是 manual gate，不是 CI blocker。

- [ ] **Step 2: 写 artifact 记录文档模板**

Create `docs/tinyui-serial/C-线人工窗口验收记录.md` with sections:
...
- 复用 `check_tinyui_visible_ui.py` 的 demo target 查找思路。
...
```bash
git add tests/tinyui/runtime/check_tinyui_manual_window_artifact.py \
        docs/tinyui-serial/C-线人工窗口验收记录.md \
        tinyui/docs/demo_guide.md \
        docs/tinyui-serial/C-线计划索引.md
git commit -m "test(tinyui): add manual window artifact gate"
```
...
```bash
git add docs/tinyui-serial/C-线计划索引.md \
        docs/superpowers/specs/2026-05-26-tinyui-lingdonggui-test-architecture-design.md \
        tinyui/docs/demo_guide.md
git commit -m "docs(tinyui): define gate sync rules"
```
...
```text
请对 /Users/cys/embedded/LingDongGUI 的 TINYUI C线做只读 closeout review。
不要修改文件。
重点检查：
...
```bash
git add docs/tinyui-serial/C-线计划索引.md
git commit -m "docs(tinyui): close c-line gate hardening"
```

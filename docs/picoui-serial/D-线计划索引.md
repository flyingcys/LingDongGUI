...
- `D线` 总实施口径：`docs/superpowers/plans/2026-05-29-picoui-d-line-current-widget-completeness-implementation.md`
...
**D线目标**：把 PicoUI 从“基础控件和门禁已闭环”推进到“当前控件合同完整、事件/状态/theme/layout 语义可持续扩展，并为后续新控件建立高质量并行推进模式”。
...
- demo 只表达用户意图，只能使用 `picoui_*` API。
- 禁止通过 demo 侧 `set_size()` / `set_pos()` 或硬编码补丁掩盖 backend/layout/theme 缺口。
- 禁止把 `runtime/capture` 非空、`ctest -L picoui` 或 manual artifact 单独说成完整 UI 完成。
...
- `D线`：`.worktree/picoui-d-current-widgets`
- `F线`：`.worktree/picoui-f-new-widgets`

### D线写面

- 当前控件完整性：
  - `picoui/include/picoui/{window,label,button,checkbox,switch,slider,text,image,widget,theme,layout}.h`
  - `picoui/src/widgets/*`
...
- 新控件候选 spec：
  - `docs/superpowers/specs/*picoui-f-line*`
  - `docs/superpowers/plans/*picoui-f-line*`
- 若进入代码 vertical slice，只允许新增或修改该新控件独立文件：
  - `picoui/include/picoui/<new_widget>.h`
  - `picoui/src/widgets/<new_widget>.c`
  - `picoui/src/backend/ldgui/backend_<new_widget>.c`
  - `picoui/demo/<new_widget>*/main.c`
  - `tests/picoui/unit/test_picoui_<new_widget>.c`
  - `tests/picoui/runtime/*` 中该新 demo 的矩阵项
...
- 如果两个 worktree 都需要改 `picoui/include/picoui/picoui.h`、`tests/picoui/CMakeLists.txt` 或 runtime matrix，必须由主线程在合并阶段串行整合。
...
- 验收：
  ...
  - `tests/picoui/unit/test_picoui_layout.c` 覆盖 padding / gap / grid cell span / align / layout dirty 的支持或拒绝语义。
...
- 写面：
  - `docs/picoui-serial/D-线计划索引.md`
  - `docs/superpowers/specs/*picoui-d-line*`
...
- 验收：
  ...
  - `tests/picoui/unit/test_picoui_widgets.c` 覆盖 props 初值。
  - `check_picoui_backend_mapping.py` 仍通过。
- 写面：
...
- 验收：
  - `tests/picoui/unit/test_picoui_button_events.c` 或新建事件测试覆盖所有交互控件。
...
- 范围：
  ...
  - state/part 的支持矩阵。
- 验收：
  - `test_picoui_theme.c` 覆盖每个支持 token。
...
- 写面：
  - 只写 `docs/superpowers/specs/*picoui-new-widgets*`
  - 不改 public header 或 backend。
- 并行性：可在 `.worktree/picoui-f-new-widgets` 中并行给独立 subagent 做；只读 spec 阶段不抢 D 线写面。若 F 线进入代码 vertical slice，必须遵守上面的 D/F 写面隔离。
...
- 写面：
  - `docs/superpowers/specs/*picoui-ci-gates*`
  - 可选 `.github/workflows/*`

...
- 当前仓库硬规则：`AGENTS.md`
- 当前发布真相源：`docs/tinyui-serial/H-线计划索引.md`
- 当前 `v0.1/v0.2` 边界：`docs/tinyui-serial/J-线计划索引.md`
- 当前新控件 vertical-slice 历史参考：`docs/tinyui-serial/F-线计划索引.md`
...
3. 不把新控件 demo 通过写成整条 TINYUI 发布能力完成。
4. 不在一个阶段内并行推进两个新控件。
...
- `git diff --check` 通过
- `tinyui/docs/demo_guide.md` 已同步 `progress_bar / qrcode / progress_wheel / message_box / date_time / clock` 六个 demo 的证明边界
...
- `tinyui_message_box_set_on_confirm()` 当前已真实桥接到底层 `ldMessageBoxSetCallback()`，不再只是存字段。
...
- 当前 TINYUI `clock` 只承诺最小 `step_second` 显示合同，不承诺背景资源 API、复杂动画或高级时钟配置。
...
4. `a-02` 的目标是“新控件切片完成”，不是“顺手统一整理 TINYUI shared core”。
...
- 当前扩展版 `a-02` 做完后，TINYUI 预计从 `9 / 26` 提升到 `15 / 26`，数量覆盖率约 `57.7%`；这仍不是 LingDongGUI 全控件覆盖。

...
- `text font` 不是少一个 setter，而是缺从 `picoui_font` 到 `ldText/text_box` 的真实 resource / ownership / runtime update 合同。
...
- `Image 语义线` 已完成 design truth 冻结：
  - `docs/superpowers/specs/2026-05-30-picoui-image-semantics-line-design.md`
...
- 当前实现进展：
  ...
  - `image enabled` 现已在 `picoui_widget_set_enabled()` 上稳定维持 `reject`
...
- `List Metadata 合同线` 已完成 design truth 冻结：
  - `docs/superpowers/specs/2026-05-30-picoui-list-metadata-contract-line-design.md`
...
- 当前实现进展：
  - `test_list_widget_user_data_is_distinct_from_on_selected_cookie` 与 `test_list_style_class_and_user_data_are_metadata_only_contract` 已补入 `tests/picoui/unit/test_picoui_list.c`
  - `style_class` 的 metadata-only 测试边界已收紧为 widget/backend wrapper 存储事实，不再错误要求 copy/value 语义
  - 当前 authoritative 结论是：`picoui_widget_set_style_class()`、`picoui_widget_set_user_data()` 与 `picoui_list_create_with_props()` 的 metadata 写入路径已经满足这条线的最小合同，不需要再为 list metadata 单独补实现
- 当前阶段结论：
...
- `Text Font 合同线` 已完成设计、实现、review 与验证闭环：
  - `design truth`: `docs/superpowers/specs/2026-05-30-picoui-text-font-contract-line-design.md`
- 当前冻结结论：
  - `struct picoui_font { family, size }` 继续按描述值语义处理，不升级成 handle-only 或 token-only public contract。

...
本文是 `H11` 的第一版发布说明真相源，用于整理当前 `TINYUI first release preparation` 的用户可读文档包。

注意：本文是 `H/J` 历史阶段文档，不是当前 `a-0.6 final release` 真相源。当前最终发布结论以 `tests/tinyui/contract/tinyui_release_capability_matrix.json`、`tinyui/docs/demo_guide.md`、`docs/tinyui-serial/C-线人工窗口验收记录.md` 和 `docs/tinyui-serial/a-0.6-final-release-closeout.md` 为准。
...
- 这仍不等于“TINYUI 已完整支持 LingDongGUI”，也不等于“人工窗口验收已通过”。
...
- 本轮独立 review 抓出的 blocker 已修复：
  - `label`：`tinyui_label_create_with_props()` 已补齐 `transparent / align / background_source`。
  - `slider`：release matrix 已回调到真实 public API 边界，不再把不存在的 `tinyui_slider_get_value()` 写成 `support`。
...
- `H13` 的 latest closeout 前状态可见：
  - [H-线发布closeout前状态](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线发布closeout前状态.md)
...
- [H-线已支持控件清单](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线已支持控件清单.md)
...
- [H-线当前9控件发布合同](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线当前9控件发布合同.md)
- [H-线发布差距与LingDongGUI控件对比](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线发布差距与LingDongGUI控件对比.md)
- [C-线人工窗口验收记录](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/C-线人工窗口验收记录.md)
...
- `runtime / visible / mapping` 共享 `build/tinyui-runtime`，必须串行执行，不能并行抢同一 build 目录。
...
- [tinyui/docs/demo_guide.md](/Users/cys/embedded/LingDongGUI/tinyui/docs/demo_guide.md)
...
| 头文件 | 当前公开范围 |
| --- | --- |
| `tinyui/include/tinyui/tinyui.h` | 聚合入口，包含全部当前 public headers |
| `tinyui/include/tinyui/app.h` | 应用生命周期 |
| `tinyui/include/tinyui/window.h` | 窗口创建与基础属性 |
| `tinyui/include/tinyui/widget.h` | 通用 widget 属性、回调类型、`struct tinyui_font` |
| `tinyui/include/tinyui/layout.h` | flex/grid 布局 API 与相关枚举 |
| `tinyui/include/tinyui/theme.h` | theme、part/state/color/metric API |
| `tinyui/include/tinyui/label.h` | label 控件 |
| `tinyui/include/tinyui/button.h` | button 控件 |
| `tinyui/include/tinyui/checkbox.h` | checkbox 控件 |
| `tinyui/include/tinyui/switch.h` | switch 控件 |
| `tinyui/include/tinyui/slider.h` | slider 控件 |
| `tinyui/include/tinyui/text.h` | text 控件 |
| `tinyui/include/tinyui/image.h` | image 控件 |
| `tinyui/include/tinyui/list.h` | list 控件 |

### 命名边界

| 类别 | 允许暴露形式 | 当前 gate 约束 |
| --- | --- | --- |
| 函数 | `tinyui_*` | 轻量文本模式识别当前可见函数声明头，再断言函数名前缀 |
| 宏 | `PICOUI_*` | 头文件 guard 等公开宏必须使用 `PICOUI_` 前缀 |
| 结构体标签 | `struct tinyui_*` | 公开 `struct` 标签不得越过 `tinyui_` 边界 |
| 枚举标签 | `enum tinyui_*` | 公开 `enum` 标签不得越过 `tinyui_` 边界 |
| typedef 回调别名 | `tinyui_*` | 公开回调 typedef 名称保持 `tinyui_` 前缀 |
...
- [check_tinyui_public_api.py](/Users/cys/embedded/LingDongGUI/tests/tinyui/contract/check_tinyui_public_api.py)

## 相关文档导航

- [H-线计划索引](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线计划索引.md)
- [H-线已支持控件清单](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线已支持控件清单.md)
- [H-线当前9控件发布合同](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线当前9控件发布合同.md)
- [H-线demo-catalog](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线demo-catalog.md)
- [H-线发布测试矩阵](/Users/cys/embedded/LingDongGUI/docs/tinyui-serial/H-线发布测试矩阵.md)

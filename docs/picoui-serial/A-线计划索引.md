...
- `A线` 总设计真相源：`docs/superpowers/specs/2026-05-26-tinyui-abstraction-layer-design.md`
- `A线` 总实施口径：`docs/superpowers/plans/2026-05-26-tinyui-abstraction-layer-implementation.md`
...
- `TINYUI` 当前已经具备：
  - `A1-A7` 已按当前口径收口
  - `TINYUI -> LingDongGUI` 真实 backend 主线已经成为当前正式实现
  - `runtime/capture` 证据链已回到 smoke / 回归层级，不再冒充最终 UI 证据
  - `tinyui/src/backend/ldgui/backend_app.c` 当前仅保留 `temporary smoke path` / host harness 职责，不再承担正式 fake renderer 主输出职责

## 历史背景（已过时）

- 2026-05-26 较早阶段，主线曾存在一个明确问题：
  - `tinyui/src/backend/ldgui/backend_app.c` 一度承担过多 fake renderer 职责
  - 当时“能看到画面”不能等于“真实 backend 映射完成”
- 该问题已在 `A7` 收口中被纠偏；上面这段仅保留为历史背景，不代表当前现态

## A线目标

- **A线唯一目标**：把 `TINYUI` 从“临时可视 smoke”纠偏成“真实 `TINYUI -> LingDongGUI` backend 映射主线”。
...
- 真相源：`docs/superpowers/specs/2026-05-26-tinyui-abstraction-layer-design.md` 第 24 节
- 当前结论：
...
- 入口 plan：`docs/superpowers/plans/2026-05-26-tinyui-abstraction-layer-implementation.md`
- 对应阶段：`Stage A`
- 当前状态：**已完成**
- 已完成内容：
...
- 已完成内容：
  ...
  - `image` 已具备最小真实绑定：`tinyui_image_source -> ldImageSetImage()`
- 当前证据：
  - `ctest --test-dir build -R test_tinyui_widgets --output-on-failure` 通过
...
- 已完成内容：
  ...
  - `button/checkbox/switch/slider` 已建立 native-event-path 到 TINYUI callback/state 的真实桥接
  - `setter-path` 已收窄成状态同步优先，不再作为主事件入口
  - `test_tinyui_button_events` / `test_tinyui_widgets` 已区分 `setter-path` 与 `native-event-path` 合同
- 当前证据：
  - `ctest --test-dir build -R 'test_tinyui_widgets|test_tinyui_button_events' --output-on-failure` 通过
...
- 当前证据：
  ...
  - `python3 examples/sdl/tests/check_use_demo_capture.py --demo 5 --build-dir build/capture-demo-5` 通过
- 非 A线 blocker：
  - `python3 examples/sdl/tests/check_switch_capture_matrix.py --build-dir build/switch-capture-verify` 当前仍暴露 legacy `USE_DEMO=0` / `uiWidgetLegacy` 的 SDL demo 侧问题，按测试架构文档归类为 `examples/sdl/tests/` 行为层回归，不纳入 `TINYUI` A 线最终 closeout gate。
...
1. 维持当前 `TINYUI -> backend/ldgui -> LingDongGUI -> SDL host` 主线
...
3. 不把“窗口能弹出来”当成“TINYUI 适配完成”
4. 不把 SDL host 改造成 `TINYUI` 专属渲染系统

## 推荐阅读顺序

1. 先读 `docs/superpowers/specs/2026-05-26-tinyui-abstraction-layer-design.md` 第 24 节
2. 再读 `docs/superpowers/plans/2026-05-26-tinyui-abstraction-layer-implementation.md`
...
- `TINYUI` demo 的可见 UI 来自真实 `LingDongGUI` backend 映射，而不是手工拼图

...
- `V4` 已完成 broad gate 修复并进入收口确认：
  ...
  - 当前第六批也已落地：`tests/picoui/contract/*` 旧真相源的证据串已进一步统一到 TinyUI 口径；其中旧 `picoui_native_100_inventory.json` 当前只剩 legacy/docs 残留面，canonical native-100 truth 已落在 `tests/tinyui/contract/*`
  ...
  - 当前又完成一批 legacy thin wrapper 退场：`tests/picoui/contract/{check_picoui_public_api.py,check_picoui_release_capability_matrix.py,check_picoui_native_100_inventory.py,check_picoui_native_api_exhaustiveness.py,check_picoui_tinyui_transition_guards.py,check_picoui_widget_contract_matrix.py}` 已删除；current live checker 只走 `tests/tinyui/contract/*`
  ...
  - 本批修复点：`test_tinyui_gauge` / `test_tinyui_image` 改为 repo-relative source helper，`test_tinyui_graph` 依赖的 test support repo marker 已识别 `tests/tinyui/unit/`，`test_tinyui_qrcode` 随完整 build 刷新旧二进制，`check_tinyui_release_capability_matrix` 移除过期 `tinyui -> picoui` 证据 alias rewrite
  - 当前又完成一小批 contract 入口收口：`check_tinyui_demo_boundary` 与 `check_tinyui_widget_contract_matrix` 已直接注册到 `tests/tinyui/contract/*` canonical 脚本；`tests/picoui/contract/check_tinyui_{demo_boundary,widget_contract_matrix}.py` 当前只保留兼容转发壳
  - 当前又完成文档 truth 对齐：`线计划索引` 与 baseline/orchestration 文档已明确区分 `tests/tinyui/contract/*` canonical 入口，和 `tests/picoui/contract/*` 历史兼容/旧真相源入口
  - 当前又完成 `V5` 起步批：与 canonical 完全一致的 `tests/picoui/contract/tinyui_{transition_inventory,v21_transition_inventory,release_capability_matrix}.json` 兼容副本已退场
  - 当前又完成一小批 canonical live checker 解耦：`tests/tinyui/contract/{ldgui_public_api_inventory.json,native_api_gap_ledger.json,ldgui_public_api_expected_symbols.json}` 已承接 canonical 副本，`check_tinyui_public_api.py` 与 `check_tinyui_release_capability_matrix.py` 已改读新路径；`tests/picoui/contract/check_ldgui_public_api_inventory.py` 当前也已切到 canonical `tests/tinyui/contract/*` 输入
  - 当前又完成一小批文档 truth 收口：`picoui_tinyui_transition_inventory.json` 在 `docs/v2.1` 主线口径中已明确降级为历史文档引用资产，不再表述成 live checker 入口
  - 当前又完成一小批 native-100 truth 收口：`tests/tinyui/contract/check_tinyui_native_100_inventory.py` 与 `tests/tinyui/contract/tinyui_native_100_inventory.json` 已建立 canonical 副本；旧 `picoui_native_100_inventory.json` 当前只剩 legacy/docs 残留面
  - 当前又完成一小批 release-matrix truth 收口：canonical exhaustiveness/release-matrix truth 已落在 `tests/tinyui/contract/*`；`tinyui/docs/*` 与 `docs/ability/*` 当前口径也已切到 canonical matrix，旧 `picoui_release_capability_matrix.json` 当前只剩 legacy/docs 残留面
  ...
  - 当前又完成一小批 unit/support umbrella 收口：`tests/support/picoui_test_support.h` 与剩余 `tests/tinyui/unit/*` umbrella consumer 当前已切到 `tinyui.h`；同时已补齐 `tinyui/include/port/sdl.h` 顶层转发头，`test_tinyui_port_sdl.c` 当前已切到 `tinyui.h + port/sdl.h`
  - 当前又完成一小批 direct widget header 收口：`tests/tinyui/unit/{test_tinyui_animation,test_tinyui_arc,test_tinyui_gauge,test_tinyui_list,test_tinyui_progress_wheel}.c` 当前已从 `picoui/<widget>.h` 切到顶层 `{animation,arc,gauge,list,progress_wheel}.h`；对应 focused unit 与 target build proof 已通过
  - 当前又完成一小批 demo/port consumer 收口：`tinyui/demo/basic_widgets/main.c` 当前已从 `picoui/*` include 集合切到顶层 `{button,checkbox,image,layout,runtime,slider,switch,text,widget,window}.h`；`tinyui/port/sdl/sdl.c` 当前也已切到 `port/sdl.h`，对应 demo/port focused build 与 `test_tinyui_port_sdl` 已通过
  - 当前又完成 widgets 实现层第一批收口：`tinyui/src/widgets/{animation,background,button,calendar,checkbox,graph,image,label,progress_bar,text}.c` 当前已从 `picoui/<widget>.h` 切到顶层同名头；对应 focused build 与相关 unit/demo proof 已通过
  - 当前又完成 widgets 实现层第二批收口：`tinyui/src/widgets/{clock,date_time,list,qrcode,window}.c` 当前已从 `picoui/*` include 集合切到顶层 `{clock,date_time,list,qrcode,widget,window}.h`；对应 focused build 与相关 unit/demo proof 已通过
  - 当前又完成 widgets 实现层第三批收口：`tinyui/src/widgets/{arc,gauge,icon_slider,message_box,radial_menu}.c` 当前已从 `picoui/*` include 集合切到顶层 `{arc,gauge,icon_slider,message_box,radial_menu,widget}.h`；对应 focused build 与相关 unit/demo proof 已通过
  - 当前又完成 widgets 实现层第四批收口：`tinyui/src/widgets/{canvas,combo_box,keyboard,line_edit,progress_wheel,scroll_selecter,slider,switch,table}.c` 当前也已从 `picoui/*` include 集合切到顶层 `{canvas,combo_box,keyboard,line_edit,progress_wheel,scroll_selecter,slider,switch,table}.h`；对应 focused build、focused unit 与 `tinyui_basic_widgets_demo` proof 已通过
  - 当前又完成 contract 真相源第一小批 wording 对齐：`tests/tinyui/contract/{native_api_gap_ledger.json,tinyui_release_capability_matrix.json}` 与 `tests/picoui/contract/native_api_gap_ledger.json` 当前已把 header evidence wording 从 `picoui/include/picoui/*.h` 同步到 `tinyui/include/*.h`；对应 canonical / legacy release-matrix 与 exhaustiveness gate 已重新通过
  - 当前又完成 canonical transition guard baseline 刷新：`tests/tinyui/contract/tinyui_v21_transition_inventory.json` baseline 当前已更新到 `tinyui_public_header_count=44` 与 `compat_public_headers_require_followup={picoui.h,sdl.h}`；current live checker 只走 `check_tinyui_v21_transition_guards.py`
  - 当前又完成 backend bridge include 收口第一步：`tinyui/src/backend/ldgui/backend.h` 当前已从 `picoui/{image,native,widget,window}.h` 切到顶层 `{image,native,widget,window}.h`；`check_tinyui_v21_transition_guards.py` baseline 当前新增 `backend_compat_includes=[]` 守卫，并与 focused build / focused unit proof 一起重新通过
  - 当前又完成 top-level wrapper baseline 钉住：`check_tinyui_v21_transition_guards.py` baseline 当前新增 `top_level_wrapper_forward_count=43` 与 `top_level_wrapper_forward_names=*`，机器真相已明确记录哪些顶层头仍直接 `#include "picoui/..."`；`check_picoui_tinyui_transition_guards.py` 兼容壳也已重新通过
  - 当前又完成 canonical widget-contract checker 口径切换：`check_tinyui_widget_contract_matrix.py` 当前已切到 `tinyui/include/*.h` 根路径，并通过单层 wrapper resolve 继续解析承载声明的 `picoui/*.h` 真头；legacy wrapper `check_picoui_widget_contract_matrix.py` 已重新通过
  - 当前又完成 legacy widget-contract checker 退场：`tests/picoui/contract/check_picoui_widget_contract_matrix.py` 当前已删除；canonical checker 只保留 `tests/tinyui/contract/check_tinyui_widget_contract_matrix.py`
  - 当前又完成 native-api-exhaustiveness residue 收口：`tests/tinyui/contract/check_tinyui_native_api_exhaustiveness.py` 当前已建立 canonical 入口，`tests/picoui/contract/check_picoui_native_api_exhaustiveness.py` 当前也已删除
  - 当前又完成 top-level wrapper 第一批真实迁出：`tinyui/include/qrcode.h` 当前已不再 forward 到 `picoui/qrcode.h`，而是直接承载 canonical public 声明；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=43` 收窄到 `42`，同时 `check_tinyui_public_api.py` 已新增“顶层头自身 include guard 合法”口径以承接这类 canonical 实头
  - 当前又完成 top-level wrapper 第二批真实迁出：`tinyui/include/{app,display,indev,osal,tick,port}.h` 当前也已不再 forward 到 `picoui/*`，而是直接承载 canonical public 声明或 canonical 聚合 include；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=42` 收窄到 `36`
  - 当前又完成 top-level wrapper 第三批真实迁出：`tinyui/include/{background,arc,gauge,icon_slider,progress_wheel,radial_menu}.h` 当前也已不再 forward 到 `picoui/*`，而是直接承载 canonical public 声明；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=36` 收窄到 `30`
  - 当前又完成 compat include chain rewiring：`tinyui/include/picoui/widget.h` 当前已把内部 include 从 `picoui/{layout,native,theme}.h` 切到顶层 `{layout,native,theme}.h`；这一步不改变 top-level wrapper 数量，但已为后续 `widget/layout/native/theme` 高耦合退场线清掉组合头回流路径
  - 当前又完成 top-level wrapper 第四批真实迁出第一步：`tinyui/include/layout.h` 当前已不再 forward 到 `picoui/layout.h`，且 `tinyui/include/picoui/{widget,line_edit}.h` 当前也已改为显式走顶层 `../layout.h` / `../native.h` / `../theme.h`；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=30` 收窄到 `29`
  - 当前又完成 top-level wrapper 第四批真实迁出第二步：`tinyui/include/{native,theme}.h` 当前已不再 forward 到 `picoui/{native,theme}.h`，且 `tinyui/include/picoui/{canvas,table,widget}.h` 当前也已改为显式走顶层 `../native.h` / `../theme.h`；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=29` 收窄到 `27`
  - 当前又完成 top-level wrapper 第四批真实迁出第三步：`tinyui/include/{progress_bar,clock}.h` 当前已不再 forward 到 `picoui/{progress_bar,clock}.h`；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=27` 收窄到 `25`
  - 当前又完成 top-level wrapper 第四批真实迁出第四步：`tinyui/include/{button,checkbox,label,text,graph}.h` 当前已不再 forward 到对应 `picoui/*.h`；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=25` 收窄到 `20`，`tinyui_public_api_count` 当前也已从 `26` 刷新到 `28`
  - 当前又完成 top-level wrapper 第四批真实迁出第五步：`tinyui/include/window.h` 当前已不再 forward 到 `picoui/window.h`，且 `tinyui/include/picoui/{button,label,switch}.h` 当前也已改为显式走顶层 `../window.h`；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=20` 收窄到 `19`
  - 当前又完成 compat widget include chain rewiring：`tinyui/include/picoui/{animation,button,calendar,checkbox,combo_box,date_time,graph,image,keyboard,label,line_edit,list,message_box,progress_bar,runtime,scroll_selecter,slider,switch,table,text}.h` 当前已从 `picoui/widget.h` 切到顶层 `../widget.h`；这一步不减少 top-level wrapper 数量，但已清掉 `widget/obj/runtime` 退场线的大部分 compat 回流面，同时 `tinyui/include/button.h` 当前也已补齐 `picoui_button_{set,get}_text_color` public 声明，`test_tinyui_widgets` 已重新通过
  - 当前又完成 top-level wrapper 第四批真实迁出第六步：`tinyui/include/{widget,obj}.h` 当前已不再 forward 或直连 `picoui/widget.h`，分别改为 canonical public 声明与顶层 `widget.h` 复用；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=19` 收窄到 `17`，focused proof 已确认当前 direct `picoui/widget.h` 代码依赖只剩 legacy umbrella `tinyui/include/picoui/picoui.h`
  - 当前又完成 legacy umbrella 第一小步收口：`tinyui/include/picoui/picoui.h` 当前已把 direct `widget/window` include 切到顶层 `../widget.h` 与 `../window.h`；focused proof 已确认当前主线代码面不再存在 direct `#include "picoui/widget.h"` / `#include "picoui/window.h"` 残留，下一步可以直接转向 `runtime.h/screen.h`
  - 当前又完成 top-level wrapper 第四批真实迁出第七步：`tinyui/include/{runtime,screen,core}.h` 当前已不再 forward 或直连 `picoui/runtime.h`，分别改为 canonical runtime 声明与顶层 `runtime.h` 复用；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=17` 收窄到 `14`，`tinyui_public_api_count` 当前也已从 `28` 刷新到 `30`，focused proof 已确认当前 direct `picoui/runtime.h` 代码依赖只剩 legacy umbrella `tinyui/include/picoui/picoui.h`
  - 当前又完成 legacy umbrella 第二小步收口：`tinyui/include/picoui/picoui.h` 当前已把最后一条 direct `runtime` 入口切到顶层 `../runtime.h`；focused proof 已确认当前主线代码面不再存在 direct `#include "picoui/{widget,window,runtime}.h"` 残留，下一步可直接转向剩余 `14` 个 top-level wrapper
  - 当前又完成剩余 wrapper 第一小批：`tinyui/include/animation.h` 当前已不再 forward 到 `picoui/animation.h`，`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=14` 收窄到 `13`；focused proof `test_tinyui_{animation,widgets}` 已通过。同时，当前又摸清了下一批边界：`image.h` 受 `picoui/canvas.h -> picoui/image.h` 组合面影响，必须和 `canvas.h` 同批处理
  - 当前又完成剩余 wrapper 第二小批：`tinyui/include/{combo_box,date_time}.h` 当前已不再 forward 到 `picoui/{combo_box,date_time}.h`，`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=13` 收窄到 `11`；focused proof `test_tinyui_{combo_box,date_time,widgets}` 已通过，当前下一批边界保持不变：`image.h` 继续必须和 `canvas.h` 同批处理
  - 当前又完成剩余 wrapper 第三小批：`tinyui/include/{keyboard,line_edit,list}.h` 当前已不再 forward 到 `picoui/{keyboard,line_edit,list}.h`，`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=11` 收窄到 `8`；focused proof `test_tinyui_{keyboard,line_edit,list,widgets}` 已通过，当前下一批边界保持不变：`image.h` 继续需要和 `canvas.h` 联批处理
  - 当前又完成剩余 wrapper 第四小批：`tinyui/include/{message_box,scroll_selecter,slider,table}.h` 当前已不再 forward 到 `picoui/{message_box,scroll_selecter,slider,table}.h`，`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=8` 收窄到 `4`；focused proof `test_tinyui_{message_box,scroll_selecter,slider,table,widgets}` 已通过，当前剩余 wrapper 已收窄到 `calendar.h / canvas.h / image.h / switch.h`
  - 当前又完成剩余 wrapper 第五小批：`tinyui/include/switch.h` 当前已不再 forward 到 `picoui/switch.h`，并保留 `tinyui_switch_{create,set_checked,is_checked,set_on_toggled}` helper；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=4` 收窄到 `3`，`tinyui_public_api_count` 当前也已从 `30` 刷新到 `31`；focused proof `test_tinyui_{switch,widgets}` 已通过，当前剩余 wrapper 已收窄到 `calendar.h / canvas.h / image.h`
  - 当前又完成 `canvas+image` 联批摸底：最小联动集合已确认是 `tinyui/include/{canvas,image}.h + tinyui/include/picoui/canvas.h`；若不同时改 `picoui/canvas.h`，`tinyui.h` 组合面会经 `canvas.h -> picoui/canvas.h -> picoui/image.h` 与顶层 `image.h` 重叠，阻塞点已收窄到这一条 compat include 链
  - 当前又完成 `canvas+image` 联批本体：`tinyui/include/{canvas,image}.h` 当前已不再 forward 到 `picoui/{canvas,image}.h`，`tinyui/include/picoui/canvas.h` 当前也已改为走顶层 `../image.h`；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=3` 收窄到 `1`；focused proof `test_tinyui_{canvas,image,animation,clock,widgets}` 已通过，当前剩余 wrapper 已只剩 `calendar.h`
  - 当前又完成最后一个 top-level wrapper：`tinyui/include/calendar.h` 当前已不再 forward 到 `picoui/calendar.h`，并改为直接承载 canonical public 声明；`check_tinyui_v21_transition_guards.py` baseline 当前已从 `top_level_wrapper_forward_count=1` 收窄到 `0`，focused proof `test_tinyui_{calendar,widgets}` 已通过，当前 top-level wrapper 已清零
  - 当前又完成 canonical public-api checker 口径切换：`check_tinyui_public_api.py` 当前已切到 `tinyui/include/*.h` 根路径，并只扫描与 legacy compat surface 同名的顶层头，再通过单层 wrapper resolve 继续解析承载声明的 `picoui/*.h` 真头；旧 `check_picoui_public_api.py` 当前已退场
  ...
  - 当前又完成一批 legacy compat shell 退场：`tests/picoui/contract/` 下那组与 canonical 完全一致的 `check_tinyui_*.py` 重复壳已删除，当前 CMake / live checker 只走 `tests/tinyui/contract/*` 与少量 `check_picoui_*.py` thin wrapper
  - 当前又完成 demo-boundary residue 退场：`tests/picoui/contract/check_picoui_demo_boundary.py` 已删除；current live checker 只保留 canonical `tests/tinyui/contract/check_tinyui_demo_boundary.py`
  - 当前又完成了一小批 legacy asset 定性：`tests/picoui/contract/README.md` 已明确旧 `ldgui_public_api_*` / `native_api_gap_ledger.json` / `picoui_release_capability_matrix.json` / `picoui_native_100_inventory.json` / `native_100_fragments/*.json` 仅保留历史资产职责，不再属于 `v2.1` current truth；后续只需继续清理 docs 历史记录里的 `test_picoui_*` / `check_picoui_*` 残留 wording
  - 当前又补齐了 `V5 Task 1 Step 1` 的真实解释：`rg -n "picoui" tinyui tests cmake docs/v2.1` 的 expected 不能再写成机械 `zero hits`；`v2.1` 当前明确允许三类命中继续存在：`tinyui/include/picoui/*` compatibility subtree、`tests/picoui/*` 历史兼容/历史资产说明、以及 release matrix 已写明仍处过渡态的 public C API / demo / runtime `picoui_*` 符号
  - 当前又完成了一批 current-facing `docs/ability/*` 口径收口：`docs/ability/README.md` 与各控件页总述已统一改成 `TinyUI 当前覆盖` / `current public API` 口径，不再把 `picoui_*` 过渡态 public C 符号误写成整套能力文档的当前主叙事；表格中的 `picoui_*` 列名与符号值继续保留为真实过渡态证据
  ...
  - 当前又完成了一小批 `docs/picoui-serial/*` 历史线边界补注：`H-线发布测试矩阵.md` 与 `H-线计划索引.md` 现在都明确只对应 PicoUI `H10/J7` 历史发布线，不再允许把其中的 `tests/picoui/*` / `check_picoui_*` / `picoui_release_capability_matrix.json` 误读成当前 `v2.1` canonical truth
  - 当前又完成了一小批 `docs/picoui-serial/a-0.x*` 历史线入口降级：`a-0.4/a-0.5/a-0.7~a-0.12/a-0.14` 索引里的“当前机器真相源入口 / 当前 final release 真相源入口”现在都已改成“本线历史真相源入口”，`a-0.6-final-release-closeout.md` 的“当前真相源入口”也已明确只对应 `a-0.6` 历史 truth-source
  - 当前又完成了一小批 `a-0.3` 历史语境降级：`docs/superpowers/specs/plans` 里的 `a-0.3 truth-source / current-15` 目标语句现在都已补齐“只对应 `2026-05-31 / a-0.3` 阶段语境”的边界说明，`docs/picoui-serial/a-0.3/README.md` 中“当前主仓 truth-source 入口”也已改成“`a-0.3` 当时主仓 truth-source 入口”
  ...
  - 当前又完成了一小批 `docs/superpowers/*` 零散语境收口：`2026-06-01-picoui-a-0-3-a-0-4-a-0-5-deep-review.md` 里的 `当前 machine truth-source / 当前 truth-source` 表述，以及 `2026-06-01-picoui-a-0-6-full-release-design.md` 里的“当前 truth-source 的边界”，现在都已限定回各自历史阶段语境
  - 当前又完成了一小批 `picoui-serial` 历史索引语境收口：`a-0.3/a-0.4/a-0.5` 索引里的 `当前 deep review` 已统一改成 `当时 deep review`，`a-0.4-a-0.6-后续版本记录.md` 与 `a-0.3-closeout-标准.md` 里悬空的 `current truth-source` 也已补上“当时阶段”锚点
  ...
  - 当前又补完了 `docs/superpowers/reviews/2026-05-31-picoui-a-0-3-closeout-review.md` 最后两处入口型措辞：`a-0.3` 只被保留为当时 `a-0.4` 历史串行线输入，“推荐的文档与执行入口”也已明确是当时语境
  - 当前又完成了一小批历史 `closeout 前状态` 锚点补注：`H-线发布closeout前状态.md`、`J-线v0.1-closeout前状态.md` 里的“当前最终发布结论”已限定回各自历史线语境，`2026-06-01-picoui-a-0-6-icon-slider-radial-menu-parity-repair.md` 里的 `release truth-source` 也已限定为 `a-0.6` 当时语境
  - `tinyui/include/picoui/*` 当前不能直接删：现已不再被 `tinyui/demo/*`、`tests/tinyui/unit/*`、`tinyui/port/*`、`tinyui/src/widgets/*` 的实现层 include，也不再被 `tinyui/src/backend/ldgui/backend.h` 直接引用；当前剩余直接服役面主要是 `13` 个 `tinyui/include/*.h` 顶层 wrapper、legacy umbrella 其余 compat include，以及 canonical public-api checker 的单层 wrapper resolve 与少量 legacy contract JSON/ledger 资产；最小安全顺序是继续按 pure thin wrapper 小批迁 header/legacy truth 面，再在 `V5` 退场
...
- `v2.1` 的完成态必须证明：
  - canonical current truth 不再把允许存在的 `picoui` 过渡态/历史资产命中误记成 residue
  - 独立 `backend/` 目录已消失
...
- completion 的真实含义已经固定：
  - no current-truth residue is still被误记为 `picoui` 主线
  - 但不要求把 release matrix 已允许的过渡态 `picoui_*` / 历史资产命中机械清零

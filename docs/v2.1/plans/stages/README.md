...
- 这一步继续保持边界：`widget-tree lifecycle seam` 已完成到 `is_kind + init_root/init_child/attach_child`，但 `picoui_list_backend_*` 仍保留为独立的 list selected-index seam，不和 shared tree lifecycle 命名批次混做

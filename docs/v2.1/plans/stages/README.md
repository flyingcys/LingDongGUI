...
- `backend_list.c` 当前仍留在 CMake 编译面，因为它还承载 production 在用的 selected-index 兼容 helper；与此同时，迁走的 widget-specific helper 实现已统一收成 `fail-closed` 返回 `-1`，避免漏网调用被 silent-success 掩盖
...
- `list` 这一小步已完成当前收紧：`backend_list.c` 里除 `selected_index/get_selected_index/sync_selected_index` 兼容链外，其余 helper 的 fail-closed 空壳边界已由 `test_picoui_list` 坐实；后续不再把它写成待确认状态
- `backend_label.c` 这批 shared helper 已完成迁离 `backend/` 路径并清零退场；`layout/keyboard/table` 仍承载实逻辑，当前仍不是低风险 compile-surface cleanup 候选
- `list` 这轮已把 `backend_list.c` 的最小保留边界进一步收紧清楚：production 继续保留的只剩 `selected_index/get_selected_index/sync_selected_index` 兼容链；`set_items/item_height/padding_group/margin_group/text_color/bg_color/select_color/align/item_widget` 这些 legacy helper 仅作为 fail-closed 空壳保留，focused test 继续要求它们既不能 silent-success，也不能改写已由 widget-local 路径建立的 native/public 状态
- `list` 兼容链新增 corrupted-binding 负向合同：`picoui_backend_list_sync_selected_index()` 在 `kind/ld_widget` 绑定损坏时必须返回 `-1`，不得把损坏状态误判成“合法未选中”，也不得污染 `list->selected_index` / `backend->value` 缓存
...
- `backend_keyboard.c` 当前继续保留，真实保留边界仍是 `input/layout/event`；这一步只收紧 retained-boundary 合同，不把它写成 compile-surface cleanup 候选
...
- `backend_event.c` 当前也已开始 shared-helper 外层剥离：`picoui_backend_emit_value_changed()`、`picoui_backend_emit_event()`、`picoui_backend_emit_clicked()` 已迁到 `tinyui/src/core/widget.c`，focused contract 由 `test_picoui_button_events` 直接锁定 callback 转发与 `cb == NULL` no-op 语义不变
- 这一小步明确不扩到 `picoui_backend_widget_dispatch_signal()` / `picoui_backend_widget_dispatch_event()`：它们仍直接承接 focus claim、data-model epoch、ld value sync、list selection rollback 与 retained native dispatch 路径，因此继续留在 `backend_event.c`，避免把“纯 emit helper 迁位”扩大成 event router 拆分
- `backend_event.c` 这一步之后仍保留的责任是真实 retained/shared event 边界：`ldMsg` bridge connect/slot、`widget_bind_host()`、`widget_bind_ld_event_bridge()`、`widget_connect_native_events()`、`widget_dispatch_signal()`、`widget_dispatch_event()`、`widget_dispatch_native_signal()`、slider/list 原生事件归一化、focus/editing 协调与 rejected-list-selection restore；本批不宣称这些已经迁完
...
- 当前 backend 编译面已进一步收缩到 7 个：`backend_app.c`、`backend_style_apply.c`、`backend_layout.c`、`backend_event.c`、`backend_keyboard.c`、`backend_table.c`、`backend_list.c`

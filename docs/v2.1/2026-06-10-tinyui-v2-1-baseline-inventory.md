...
- 当前顶层产品目录已收口为：
  - `tinyui/`
- 顶层 `tinyui/` 已物理消失；当前不再允许把产品层写成双顶层目录并存。
- 当前 canonical umbrella header 是 `tinyui/include/tinyui.h`；`tinyui/include/tinyui/*`
...
- 当前 canonical contract/test/perf 目录已收口为：
  ...
  - `tests/tinyui/perf/*`
  - `tests/tinyui/unit/*`
- `tests/tinyui/{contract,runtime,perf}` 当前只保留历史兼容入口与旧真相源验证；它们不是当前主线 canonical 执行入口。`tests/tinyui/contract/README.md` 已明确旧 JSON/ledger 仅保留历史资产职责。
...
- 当前仍保留的历史兼容/旧真相源入口：
  - `tests/tinyui/contract/tinyui_tinyui_transition_inventory.json` 当前只保留给 `docs/v2.0/*` / `docs/v2.1` 历史记录引用，不在 live checker 执行链中
  - legacy 目录下与 canonical 完全一致的 `check_tinyui_*.py` 与 `check_tinyui_*.py` thin wrapper 兼容壳已在 `V5` residue 清理批退场；当前这批旧入口只应被读取为历史文档记录，不应再视作主线 canonical truth。
...
- `tests/tinyui/runtime/*` 与 `tests/tinyui/perf/*` 当前只保留兼容验证入口，不再是主线 broad gate 口径。
...
- 当前基线文档保留的是 `V0` 冻结值与后续阶段的最新口径；`tests/tinyui/*` 旧证据线当前只应被读取为历史兼容入口，不再是主线 broad gate 口径。

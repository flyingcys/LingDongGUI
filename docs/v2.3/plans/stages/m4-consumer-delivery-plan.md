...
主线程对 `tinyui_demos_create`、`tinyui_demos_frame` 和 29 个 `tinyui_demo_<name>` 入口逐一调用 GitNexus MCP：

```text
impact({target: "<精确符号名>", direction: "upstream"})
```

预期：形成调用者迁移清单；`HIGH` 或 `CRITICAL` 结果必须先告警并记录受影响 demo runner、runtime capture 和 CTest，再允许编辑。
...
在 `tests/tinyui/CMakeLists.txt` 中保持测试名 `check_tinyui_demo_boundary`，labels 固定为 `tinyui;contract;demo;consumer`。
...
- [ ] **步骤 3：将 runner 收口到 canonical lifecycle**
...
- [ ] **步骤 2：逐个迁移 creator、setter 和 callback**
...
- [ ] **步骤 4：运行 focused boundary/build**
...
- [ ] **步骤 4：运行 focused runtime/capture**
...
- [ ] **步骤 1：先扩展 deprecated usage gate**

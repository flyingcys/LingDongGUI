...
void __arm_2d_impl_rgb565_user_draw_circle(
                            ld_arm_2d_user_draw_circle_descriptor_t *ptThis,
                               ...
                               uint8_t chOpacity)
{
    ...
    ARM_2D_IMPL(ld_arm_2d_user_draw_circle_descriptor_t, ptOP);
    ...
    OP_CORE.ptOp = &LD_ARM_2D_OP_USER_DRAW_CIRCLE;
...
{
    ARM_2D_IMPL(ld_arm_2d_user_draw_circle_descriptor_t, ptTask->ptOP);
    ...
    tTargetRegion.tLocation = arm_2d_get_absolute_location(((arm_2d_op_t *)ptThis)->Target.ptTile,
...
__WEAK
void __arm_2d_impl_rgb565_user_draw_circle(ld_arm_2d_user_draw_circle_descriptor_t *ptThis,
                                           ...
                                           arm_2d_region_t *ptTargetRegionOnVirtualScreen)
{
    ...
    q16_t q161MagicRatio = reinterpret_q16_s16((int16_t)(wRadiusBorder2 - wRadius2));
    ...
    for (int_fast16_t iY = ptValidRegionOnVirtualScreen->tLocation.iY; iY < iYLimit; iY++) {
        ...
        if (ABS(iYOffset) < this.tParams.iRadius) {
            uint32_t wDistance2 = wRadius2 - (uint32_t)iYOffset * (uint32_t)iYOffset;
            float fXDelta;
        ...
        for (; iX < iXLimit; iX++) {
...

            {
                ...
                uint32_t wDistance2 = (uint32_t)iXOffset * (uint32_t)iXOffset
...
def_low_lv_io(__ARM_2D_IO_USER_DRAW_CIRCLE_RGB565,
              __arm_2d_rgb565_sw_user_draw_circle);
...
const __arm_2d_op_info_t LD_ARM_2D_OP_USER_DRAW_CIRCLE = {
    .Info = {
        ...
        .chOpIndex = LD_ARM_2D_OP_IDX_USER_DRAW_CIRCLE,
        .LowLevelIO = {
